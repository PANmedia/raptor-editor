    ...
    while(j<tr.cells.length || k<rowspanLeft.length) {
      if (rowspanLeft[k]) {
        ...
        for(cs=Math.max(1,parseInt(td.colSpan)); cs>0; cs--) {
          if (rowspanLeft[k]) break; // Overlapping colspan and rowspan cells
          rowspanLeft[k++]=rs-1;
        }
      }
    }
  ...
  while(i<tableRows0) {
    ...
    while(k<tableCols0) {
      ...
      if (rowspanLeft[k]) {
        if (rCount<0 && i==rStart-rCount && rowspanCell[k]
          ...
          if (--rowspanLeft[k++] == 0) rowspanCell[k]=null;
    ...
    if (rCount>0 && i==rStart) {
      ...
      for(l=0; l<tableCols0; l++) {
        if (rowspanLeft[l]) rowspanLeft[l]+=rCount;
        if (rowspanCell[l]) rowspanCell[l].rowSpan+=rCount;
      }
