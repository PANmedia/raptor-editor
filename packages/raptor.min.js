...
d=[],e=0;this.eachRange(function(g){var g=g.commonAncestorContainer,f;f=g.nodeType===3?c(g).parent():c(g);g=[];for(d.push(g);f[0]&&!a.isRoot(f)&&f[0].tagName.toLowerCase()!=="body";)g.push(f),f=f.parent();g.reverse();b&&(b+=" | ");b+=this.getTemplate("root");for(f=0;f<g.length;f++)b+=this.getTemplate("tag",{element:g[f][0].tagName.toLowerCase(),data:"["+e+","+f+"]"});e++},null,this);b||(b=this.getTemplate("root"));this.path.html(b).find("a").click(function(){var b=c(this).data("ui-editor-selection");
b?(a.selectOuter(d[b[0]][b[1]]),a.updateTagTree()):a.selectOuter(a.getElement())});this.fire("tagTreeUpdated")}},isRoot:function(a){return this.getElement()[0]===c(a)[0]},unify:function(a,b){b!==false&&a(this);if(this.options.unify)for(var d=c.ui.editor.getInstances(),e=0;e<d.length;e++)d[e]!==this&&d[e].options.unify&&a(d[e])},getUniqueId:function(){return c.ui.editor.getUniqueId()},loadMessages:function(){this.messages=c(this.getTemplate("messages")).appendTo(this.wrapper)},showMessage:function(a,
b,d){var d=c.extend({},this.options.message,d),e={timer:null,editor:this,show:function(){this.element.slideDown();this.timer=k.setTimeout(function(a){this.timer=null;a.hide()},d.delay,this)},hide:function(){if(this.timer)k.clearTimeout(this.timer),this.timer=null;this.element.stop().slideUp(c.proxy(function(){c.isFunction(d.hide)&&d.hide.call(this);this.element.remove()},this))}};e.element=c(this.getTemplate("message",{type:a,message:b})).hide().appendTo(this.messages).find(".ui-editor-message-close").click(function(){e.hide()}).end();
e.show();return e},showLoading:function(a,b){return this.showMessage("clock",a,b)},showInfo:function(a,b){return this.showMessage("info",a,b)},showError:function(a,b){return this.showMessage("circle-close",a,b)},showConfirm:function(a,b){return this.showMessage("circle-check",a,b)},showWarning:function(a,b){return this.showMessage("alert",a,b)},loadToolbar:function(){var a=this.toolbar=c("<div/>").addClass(this.options.baseClass+"-toolbar"),a=this.toolbarWrapper=c("<div/>").addClass(this.options.baseClass+
"-toolbar-wrapper").append(a),b=this.path=c("<div/>").addClass(this.options.baseClass+"-path").addClass("ui-widget-header").html(this.getTemplate("root")),d=this.wrapper=c("<div/>").addClass(this.options.baseClass+"-wrapper").addClass("ui-widget-content").css("display","none").append(b).append(a);c.fn.draggable&&this.options.draggable&&(d.draggable({cancel:"a, button",cursor:"move",handle:".ui-editor-path",stop:c.proxy(function(){var a=this.persist("position",[d.css("top"),d.css("left")]);d.css({top:Math.abs(a[0]),
left:Math.abs(a[1])})},this)}),d.css("position",""),(a=this.persist("position")||this.options.dialogPosition)||(a=[10,10]),parseInt(a[0],10)+d.outerHeight()>c(k).height()&&(a[0]=c(k).height()-d.outerHeight()),parseInt(a[1],10)+d.outerWidth()>c(k).width()&&(a[1]=c(k).width()-d.outerWidth()),d.css({top:Math.abs(parseInt(a[0])),left:Math.abs(parseInt(a[1]))}));c(function(){d.appendTo("body")});this.loadUi()},isToolbarLoaded:function(){return this.wrapper!==null},showToolbar:function(a){this.isToolbarLoaded()||
this.loadToolbar();if(!this.visible){this.options.unify&&this.hideOtherToolbars(true);this.visible=true;this.wrapper.css("display","");this.fire("resize");typeof this.getElement().attr("tabindex")==="undefined"&&this.getElement().attr("tabindex",-1);if(a)if(a.select)a.select();else{var b=k.getSelection();b.removeAllRanges();b.addRange(a)}var d=this;c(function(){d.fire("show");d.getElement().focus()})}},hideToolbar:function(){if(this.visible)this.visible=false,this.wrapper.hide(),this.fire("hide"),
this.fire("resize")},hideOtherToolbars:function(a){this.unify(function(b){b.hideToolbar(a)},false)},isVisible:function(){return this.visible},getTemplate:function(a,b){var d;d=this.templates[a]?this.templates[a]:c.ui.editor.getTemplate(a,this.options.urlPrefix);d=d.replace(/_\('(.*?)'\)/g,function(a,b){b=b.replace(/\\(.?)/g,function(a,b){switch(b){case "\\":return"\\";case "0":return"\x00";case "":return"";default:return b}});return f(b)});b=c.extend({},this.options,b||{});b=this.getTemplateVars(b);
return d=d.replace(/\{\{(.*?)\}\}/g,function(a,d){return b[d]})},getTemplateVars:function(a,b,d){b=b?b+".":"";d||(d=1);var c={},g;for(g in a)if(typeof a[g]==="object"&&d<5){var f=this.getTemplateVars(a[g],b+g,++d),j;for(j in f)c[j]=f[j]}else c[b+g]=a[g];return c},historyPush:function(){if(this.historyEnabled&&this.getHtml()!==this.historyPeak()){if(this.present!==this.history.length-1)this.history=this.history.splice(0,this.present+1);this.history.push(this.getHtml());this.present=this.history.length-
1}},historyPeak:function(){return!this.history.length?null:this.history[this.present]},historyBack:function(){if(this.present>0)this.present--,this.setHtml(this.history[this.present]),this.historyEnabled=false,this.change(),this.historyEnabled=true},historyForward:function(){if(this.present<this.history.length-1)this.present++,this.setHtml(this.history[this.present]),this.historyEnabled=false,this.change(),this.historyEnabled=true},loadUi:function(){for(var a=0,b=this.options.uiOrder.length;a<b;a++){for(var d=
this.options.uiOrder[a],e=c("<div/>"),g=0,f=d.length;g<f;g++)if(!(this.options.enableUi===false&&typeof this.options.ui[d[g]]==="undefined"||this.options.ui[d[g]]===false)&&c.inArray(d[g],this.options.disabledUi)===-1){var j=d[g].replace(/([A-Z])/g,function(a){return"-"+a.toLowerCase()});if(c.ui.editor.ui[d[g]]){var l=c.extend({},c.ui.editor.ui[d[g]]),j=c.extend(true,{},this.options,{baseClass:this.options.baseClass+"-ui-"+j},l.options,this.options.ui[d[g]]);l.editor=this;l.options=j;l.ui=l.init(this,
j);l.ui.init(d[g],this,j,l).appendTo(e);this.uiObjects[d[g]]=l}}e.addClass("ui-buttonset").addClass(this.options.baseClass+"-buttonset");e.children().length>0&&e.appendTo(this.toolbar)}c("<div/>").css("clear","both").appendTo(this.toolbar)},uiButton:function(a){return c.extend({button:null,options:{},init:function(a,d,e,g){var h=a.replace(/([A-Z])/g,function(a){return"-"+a.toLowerCase()}),e=c.extend({},d.options,{baseClass:d.options.baseClass+"-"+h+"-button"},this.options,d.options.ui[a]);if(!this.title)this.title=
f("Unnamed Button");this.button=c("<div/>").html(this.label||this.title).addClass(e.baseClass).attr("name",a).attr("title",this.title).val(a);e.classes&&this.button.addClass(e.classes);this.button.bind("mousedown."+g.editor.widgetName,function(a){a.preventDefault()});var j=this;this.button.bind("mouseup."+g.editor.widgetName,function(a){a.preventDefault();j.click.apply(g,arguments)});d.bind("destroy",c.proxy(function(){this.button.button("destroy").remove()},this));this.button.button({icons:{primary:this.icon||
"ui-icon-"+h},disabled:e.disabled?true:false,text:this.text||false,label:this.label||null});this.ready.call(g);return this.button},disable:function(){this.button.button("option","disabled",true)},enable:function(){this.button.button("option","disabled",false)},ready:function(){},click:function(){}},a)},uiSelectMenu:function(a){return c.extend({select:null,selectMenu:null,button:null,menu:null,options:{},init:function(a,d){var e=this,g=a.replace(/([A-Z])/g,function(a){return"-"+a.toLowerCase()});c.extend({},
d.options,{baseClass:d.options.baseClass+g+"-select-menu"},e.options,d.options.ui[a]);if(!e.title)e.title=f("Unnamed Select Menu");e.selectMenu=c('<div class="ui-selectmenu ui-editor-selectmenu"/>');e.selectMenu.append(e.select.hide());e.menu=c('<div class="ui-selectmenu-menu ui-editor-selectmenu-menu ui-widget-content ui-corner-bottom ui-corner-tr"/>');e.select.find("option").each(function(){c("<div/>").addClass("ui-selectmenu-menu-item ui-editor-selectmenu-menu-item").addClass("ui-corner-all").html(c(this).html()).appendTo(e.menu).bind("mouseenter."+
d.widgetName,function(){c(this).addClass("ui-state-focus")}).bind("mouseleave."+d.widgetName,function(){c(this).removeClass("ui-state-focus")}).bind("click."+d.widgetName,function(){var a=e.select.find("option").eq(c(this).index());e.select.val(a.val());e.update();e.selectMenu.removeClass("ui-editor-selectmenu-visible");e.button.addClass("ui-corner-all").removeClass("ui-corner-top");e.change(e.select.val());return false})});var g=c("<div/>").addClass("ui-selectmenu-text"),h=c("<div/>").addClass("ui-icon ui-icon-triangle-1-s");
e.button=c("<div/>").addClass("ui-selectmenu-button ui-editor-selectmenu-button ui-button ui-state-default").attr("title",e.title).append(g).append(h).append(e.select).append(e.menu).prependTo(e.selectMenu);e.button.bind("click."+d.widgetName,function(){e.menu.css("min-width",e.button.outerWidth()+10);e.selectMenu.toggleClass("ui-editor-selectmenu-visible");return false});g=e.select.find("option[value="+e.select.val()+"]").html();e.button.find(".ui-selectmenu-text").html(g);return e.selectMenu},update:function(){var a=
this.select.find("option[value="+this.select.val()+"]").html();this.button.find(".ui-selectmenu-text").html(a)},val:function(){var a=this.select.val.apply(this.select,arguments);this.update();return a},change:function(){}},a)},getPlugin:function(a){return this.plugins[a]},loadPlugins:function(){if(!this.options.plugins)this.options.plugins={};for(var a in c.ui.editor.plugins)if(!(this.options.enablePlugins===false&&typeof this.options.plugins[a]==="undefined"||this.options.plugins[a]===false)&&c.inArray(a,
this.options.disabledPlugins)===-1){var b=c.extend({},c.ui.editor.plugins[a]),d=a.replace(/([A-Z])/g,function(a){return"-"+a.toLowerCase()}),d=c.extend(true,{},this.options,{baseClass:this.options.baseClass+"-"+d},b.options,this.options.plugins[a]);b.editor=this;b.options=d;b.init(this,d);this.plugins[a]=b}},isDirty:function(){return this.dirty},getHtml:function(){var a=this.getElement().html(),a=c("<div/>").html(a);a.find(".rangySelectionBoundary").remove();return a=a.html()},getCleanHtml:function(){this.fire("clean");
var a=this.getElement().html();this.fire("restore");a=c("<div/>").html(a);a.find(".rangySelectionBoundary").remove();return a=a.html()},setHtml:function(a){this.getElement().html(a);this.fire("html");this.change()},resetHtml:function(){this.setHtml(this.getOriginalHtml())},getOriginalHtml:function(){return this.originalHtml},save:function(){var a=this.getCleanHtml();this.fire("save");this.setOriginalHtml(a);this.fire("saved");this.fire("change");return a},setOriginalHtml:function(a){this.originalHtml=
a},bind:function(a,b,d){var e=this.events;c.each(a.split(","),function(a,f){f=c.trim(f);e[f]||(e[f]=[]);e[f].push({context:d,callback:b})})},unbind:function(a,b,d){for(var c=0,g=this.events[a].length;c<g;c++)this.events[a][c]&&this.events[a][c].callback===b&&this.events[a][c].context===d&&this.events[a].splice(c,1)},fire:function(a,b,d){d||this.fire("before:"+a,b,true);if(this.events[a])for(var e=0,g=this.events[a].length;e<g;e++){var f=this.events[a][e];f.callback.call(f.context||this)}b!==false&&
c.ui.editor.fire(a);d||this.fire("after:"+a,b,true)}});c.extend(c.ui.editor,{defaults:{plugins:{},ui:{},bind:{},domTools:{savedSelection:false,saveSelection:function(){this.savedSelection=i.saveSelection()},restoreSelection:function(){if(this.savedSelection)i.restoreSelection(this.savedSelection),this.savedSelection=false},eachRange:function(a,b,d){for(var b=b||i.getSelection(),d=d||this,c,g=0;c=b.getAllRanges()[g++];)a.call(d,c)},constrainSelection:function(a,b){var a=c(a)[0],b=b||i.getSelection(),
d;c(b.getAllRanges()).each(function(e,g){d=this.commonAncestorContainer.nodeType===3?c(g.commonAncestorContainer).parent()[0]:g.commonAncestorContainer;a!==d&&!c.contains(a,d)&&b.removeRange(g)})},getSelectedElements:function(){var a=new jQuery;this.eachRange(function(b){a.push(this.getSelectedElement(b)[0])});return a},getSelectedElement:function(a){return c(a.commonAncestorContainer.nodeType===3?a.commonAncestorContainer.parentNode:a.commonAncestorContainer)},unwrapParentTag:function(a){this.getSelectedElements().each(function(){c(this).is(a)&&
c(this).replaceWith(c(this).html())})},wrapTagWithAttribute:function(a,b,d){this.eachRange(function(c){c=this.getSelectedElement(c);c.is(a)?c.attr(b):this.toggleWrapper(a,{classes:d,attributes:b})})},selectInner:function(a,b){b=b||i.getSelection();b.removeAllRanges();c(a).focus().contents().each(function(){var a=i.createRange();a.selectNodeContents(this);b.addRange(a)})},selectOuter:function(a,b){b=b||i.getSelection();b.removeAllRanges();c(a).each(function(){var a=i.createRange();a.selectNode(this);
b.addRange(a)}).focus()},toggleWrapper:function(a,b){var b=b||{},d=i.createCssClassApplier(b.classes||"",{normalize:true,elementTagName:a,elementProperties:b.attributes||{}});this.eachRange(function(e){this.rangeEmptyTag(e)?this.replaceRange(c("<"+a+"/>").addClass(b.classes).attr(b.attributes||{}).append(this.domFragmentToHtml(e.cloneContents())),e):d.toggleRange(e)})},rangeEmptyTag:function(a){a=this.domFragmentToHtml(a.cloneContents());typeof a==="string"&&(a=a.replace(/([ #;&,.+*~\':"!^$[\]()=>|\/@])/g,
"\\$1"));return c(a).is(":empty")?true:false},execCommand:function(a,b,c){document.execCommand(a,b,c)},insertTag:function(a,b){this.eachRange(function(b){b.insertNode(c("<"+a+"/>")[0])},b)},insertTagAtEnd:function(a,b){this.eachRange(function(b){b.insertNodeAtEnd(c("<"+a+"/>")[0])},b)},insertElement:function(a,b,d){this.eachRange(function(d){c(a).each(function(){d.insertNode(b===false?this:this.cloneNode(true))})},d)},insertElementAtEnd:function(a,b,d){this.eachRange(function(d){c(a).each(function(){d.insertNodeAtEnd(b===
false?this:this.cloneNode(true))})},d)},toggleBlockStyle:function(a,b){this.eachRange(function(d){for(d=c(d.commonAncestorContainer);d.length&&d[0]!==b[0]&&(d[0].nodeType===3||d.css("display")==="inline");)d=d.parent();d[0]===b[0]&&b.css("display")!=="inline"&&(this.wrapInner(b,"div"),d=b.children().first());this.toggleStyle(d,a)})},wrapInner:function(a,b){this.saveSelection();c(a).each(function(){var d=c("<"+b+"/>").html(c(this).html());a.html(d)});this.restoreSelection()},wrapRange:function(a){a.replaceContents()},
inverseWrapWithTagClass:function(a,b,d,e){this.saveSelection();var g="domTools"+Math.ceil(Math.random()*1E7);this.eachRange(function(a){var c=i.createCssClassApplier(e,{elementTagName:d});c.isAppliedToRange(a)?c.toggleSelection():i.createCssClassApplier(b,{elementTagName:g}).toggleSelection()});c(g).each(function(){c(this).replaceWith(c("<"+a+"/>").addClass(b).html(c(this).html()))});this.restoreSelection()},toggleStyle:function(a,b){c.each(b,function(b,e){c(a).css(b)===e?c(a).css(b,""):c(a).css(b,
e)})},getStyles:function(a){for(var b={},a=k.getComputedStyle(a[0],null),c=0;c<a.length;c++)b[a.item(c)]=a.getPropertyValue(a.item(c));return b},swapStyles:function(a,b,c){for(var e in c)a.css(e,b.css(e)),b.css(e,c[e])},replaceSelection:function(a,b){this.eachRange(function(b){this.replaceRange(a,b)},b)},replaceRange:function(a,b){var d=c("<div></div>").append(a)[0].childNodes;b.deleteContents();d.length===p||d.length===1?b.insertNode(d[0].cloneNode(true)):c.each(d,function(a,c){b.insertNodeAtEnd(c.cloneNode(true))})},
insertDomFragmentBefore:function(a,b,d){for(var e=0,g=a.childNodes.length;e<g;e++){var f=a.childNodes.item(e);(f=f.nodeType===3?f.nodeValue:c(f).html())&&c("<"+d+"/>").html(c.trim(f)).insertBefore(b)}},domFragmentToHtml:function(a,b){for(var d="",e=0,g=a.childNodes.length;e<g;e++){var f=a.childNodes.item(e);(f=f.nodeType===3?f.nodeValue:this.outerHtml(f))&&(d+=f)}b&&(d=c("<"+b+">"+d+"</"+b+">"),d.find("p").wrapInner("<"+b+"/>"),d.find("p > *").unwrap(),d=c("<div></div>").html(d).html());return d},
selectionExists:function(a){var b=false;this.eachRange(function(a){this.isEmpty(a)||(b=true)},a);return b},isEmpty:function(a){return a.startOffset===a.endOffset&&a.startContainer===a.endContainer},expandToParent:function(a){a.setStartBefore(a.startContainer);a.setEndAfter(a.endContainer)},changeTag:function(a,b){this.insertDomFragmentBefore(a.extractContents(),a.startContainer,b);c(a.startContainer).remove()},tagSelection:function(a,b){this.eachRange(function(b){this.isEmpty(b)?(this.expandToParent(b),
this.changeTag(b,a)):this.replaceRange(this.domFragmentToHtml(b.extractContents(),a),b)},b)},outerHtml:function(a){return c(a).clone().wrap("<div></div>").parent().html()}},namespace:null,unify:true,persistence:true,persistenceName:"uiEditor",unloadWarning:true,autoEnable:false,partialEdit:false,enablePlugins:true,disabledPlugins:[],uiOrder:null,enableUi:true,disabledUi:[],message:{delay:5E3},replace:false,replaceStyle:"display,position,float,width,padding-left,padding-right,padding-top,padding-bottom,margin-left,margin-right,margin-top,margin-bottom".split(","),
...
getTemplate:function(a,b){var d;if(this.templates[a])d=this.templates[a];else{var e=b||this.urlPrefix,g=a.split(".");e+=g.length===1?"templates/"+g[0]+".html":"plugins/"+g[0]+"/templates/"+g.splice(1).join("/")+".html";c.ajax({url:e,type:"GET",async:false,timeout:15E3,error:function(){d=null},success:function(a){d=a}});this.templates[a]=d}return d},getUniqueId:function(){for(var a=c.ui.editor.defaults.baseClass+"-uid-"+(new Date).getTime()+"-"+Math.floor(Math.random()*1E5);c("#"+a).length;)a=c.ui.editor.defaults.baseClass+
...
a+'="'+this.options.stripAttrs[a]+'"]').removeAttr(this.options.stripAttrs[a]);for(a=0;a<this.options.stripEmptyTags.length;a++)b.getElement().find(this.options.stripEmptyTags[a]).filter(function(){if(c.trim(c(this).html())!=="")return false;if(!c(this).hasClass("rangySelectionBoundary"))return true;if(b.savedSelection!==false)return false}).remove();for(a=0;a<this.options.stripEmptyAttrs.length;a++){var d=this.options.stripEmptyAttrs[a];b.getElement().find("["+this.options.stripEmptyAttrs[a]+"]").filter(function(){return c.trim(c(this).attr(d))===
""}).removeAttr(this.options.stripEmptyAttrs[a])}}});c.ui.editor.registerUi({clean:{init:function(a){return a.uiButton({title:f("Remove unnecessary markup from editor content"),click:function(){a.getPlugin("clean").clean()}})}}});c.ui.editor.registerPlugin("clickToEdit",{init:function(a,b){var d=this,e=c(a.getTemplate("clicktoedit.message",b)).appendTo("body"),b=c.extend({},{obscureLinks:false,position:{at:"center center",of:a.getElement(),my:"center center",using:function(a){c(this).css({position:"absolute",
...
"-hover");e.removeClass(b.baseClass+"-visible")};this.edit=function(){d.hide();a.isEditing()||a.enableEditing();a.isVisible()||a.showToolbar(d.selection())};e.position(b.position);b.obscureLinks||(a.getElement().find("a").bind("mouseenter."+a.widgetName,d.hide),a.getElement().find("a").bind("mouseleave."+a.widgetName,d.show));a.getElement().bind("mouseenter."+a.widgetName,d.show);a.getElement().bind("mouseleave."+a.widgetName,d.hide);a.getElement().bind("click."+a.widgetName,function(a){(b.obscureLinks||
!c(a.target).is("a")&&!c(a.target).parents("a").length)&&d.edit()});a.bind("destroy",function(){e.remove();a.getElement().unbind("mouseenter."+a.widgetName,d.show);a.getElement().unbind("mouseleave."+a.widgetName,d.hide);a.getElement().unbind("click."+a.widgetName,d.edit)})}});c.ui.editor.registerPlugin("dock",{enabled:false,docked:false,topSpacer:null,bottomSpacer:null,options:{docked:false,dockToElement:false,dockUnder:false,persist:true,persistID:null},init:function(){this.bind("show",this.show);
this.bind("hide",this.hide);this.bind("disabled",this.disable);this.bind("destroy",this.destroy,this)},show:function(){this.enabled?this.isDocked()&&this.showSpacers():((this.loadState()||this.options.docked)&&this.dock(),this.enabled=true)},hide:function(){this.hideSpacers();this.editor.toolbar.css("width","auto")},showSpacers:function(){if(!this.options.dockToElement&&this.editor.toolbar.is(":visible"))this.topSpacer=c("<div/>").addClass(this.options.baseClass+"-top-spacer").height(this.editor.toolbar.outerHeight()).prependTo("body"),
this.bottomSpacer=c("<div/>").addClass(this.options.baseClass+"-bottom-spacer").height(this.editor.path.outerHeight()).appendTo("body"),this.editor.fire("resize")},hideSpacers:function(){if(this.topSpacer)this.topSpacer.remove(),this.topSpacer=null;if(this.bottomSpacer)this.bottomSpacer.remove(),this.bottomSpacer=null;this.editor.fire("resize")},swapStyle:function(a,b,c){var e={},f;for(f in c)a.css(f,b.css(f)),e[f]=b.css(f),b.css(f,c[f]);return e},revertStyle:function(a,b){for(var c in b)a.css(c,
...
this,d=this.persist("locale");d&&n(d);var d=c('<select autocomplete="off" name="i18n"/>'),e;for(e in q){var g=c('<option value="'+e+'" class="'+e+'"/>');g.html(t[e]);m===e&&g.attr("selected","selected");d.append(g)}return a.uiSelectMenu({title:f("Change Language"),select:d,change:function(a){n(b.persist("locale",a))}})}}});o("en","English",{});o("zh_CN","\u7b80\u4f53\u4e2d\u6587",{"Added link: {{link}}":"Added link: {{link}}","All changes will be lost!":"All changes will be lost!","Apply Source":"\u5e94\u7528\u6e90\u4ee3\u7801",
...
root:"\u672c","{{images}} image(s) have been replaced with resized versions":"{{images}} image(s) have been replaced with resized versions"});c.ui.editor.registerPlugin("imageResize",{options:{resizeAjax:false,resizingClass:"",resizeAjaxClass:"",ajax:{url:"/",type:"post",cache:false}},init:function(a){this.options=c.extend(this.options,{resizingClass:this.options.baseClass+"-in-progress",resizeAjaxClass:this.options.baseClass+"-on-save"});a.bind("enabled",this.bind,this);this.resized=c.proxy(this.imageResized,
this);var b=this;a.getElement().find("img").each(function(){this.addEventListener&&this.addEventListener("DOMAttrModified",b.resized,false);this.attachEvent&&this.attachEvent("onpropertychange",b.resized)})},bind:function(){this.editor.bind("change",this.scanImages,this);this.editor.bind("destroy",this.destroy,this);this.editor.bind("save",this.save,this);this.editor.bind("cancel",this.cancel,this)},imageResized:function(a){var b=c(a.target);b.is("img")&&b.attr("_moz_resizing")&&a.attrName=="style"&&
a.newValue.match(/width|height/)&&(this.options.resizeAjax&&b.addClass(this.options.resizeAjaxClass),this.editor.fire("change"))},scanImages:function(){var a=this.editor.getElement(),b=[];c(a.find("img")).each(function(){(a.height()<c(this).outerHeight()||a.width()<c(this).outerWidth())&&b.push(c(this))});b.length&&this.resizeImageElements(b,a.width(),a.height())},resizeImageElements:function(a,b,d){for(var e=c("<a>",{href:"",target:"_blank"}),f=0;f<a.length;f++){var h=a[f],j=h.outerWidth(),l=h.outerHeight(),
i=Math.min(b/j,d/l),j=Math.abs(i*(j-(h.outerWidth()-h.width()))),l=Math.abs(i*(l-(h.outerHeight()-h.height())));h.addClass(this.options.resizingClass);e=e.html(h.attr("title")||h.attr("src").substr(h.attr("src").lastIndexOf("/")+1)).attr("href",h.attr("src"));c(h).css({width:j,height:l}).attr("height",l).attr("width",j);this.options.resizeAjax&&h.addClass(this.options.resizeAjaxClass);var k=this;this.showOversizeWarning(this.editor.outerHtml(e),{hide:function(){h.removeClass(k.options.resizingClass)}})}},
cancel:function(){this.removeClasses();this.editor.unbind("change",this.scanImages,this);var a=this;this.editor.getElement().find("img").each(function(){this.removeEventListener&&this.addEventListener("DOMAttrModified",a.resized,false);this.detachEvent&&this.detachEvent("onpropertychange",a.resized)})},destroy:function(){this.cancel()},save:function(){this.removeClasses(this.options.resizingClass);this.options.resizeAjax&&this.resizeImagesAjax()},resizeImagesAjax:function(){var a=this,b=[];c("."+
this.options.resizeAjaxClass).each(function(){c(this).attr("id")||c(this).attr("id",a.editor.getUniqueId());b.push({id:c(this).attr("id"),src:this.src,width:c(this).width(),height:c(this).height()})});if(b.length){var d=this.editor.showLoading(f("Resizing image(s)")),e=this.options.ajax;e.data={images:b};e.async=false;c.ajax(e).done(function(e){c(e).each(function(){c("#"+this.id).attr("src",this.src).removeAttr("id")});a.editor.fire("change");d.hide();a.editor.showConfirm(f("{{images}} image(s) have been replaced with resized versions",
{images:b.length}))}).fail(function(b){d.hide();a.editor.showError(f("Failed to resize images (error {{error}})",{error:b.status}))}).always(function(){a.removeClasses([a.options.resizeAjaxClass])})}},showOversizeWarning:function(a,b){this.options.resizeAjax?this.editor.showInfo(f('The image "{{image}}" is too large for the element being edited.<br/>It will be replaced with a resized copy when your edits are saved.',{image:a}),b):this.editor.showInfo(f('The image "{{image}}" is too large for the element being edited.<br/>It has been resized with CSS.',
{image:a}),b)},removeClasses:function(a){a||(a=[this.options.resizingClass,this.options.resizeAjaxClass]);c.isArray(a)||(a=[a]);for(var b=0;b<a.length;b++)this.editor.getElement().find("img."+a[b]).removeClass(a[b])}});c.ui.editor.registerUi({length:{ui:null,options:{length:150},init:function(a){a.bind("show",c.proxy(this.updateCount,this));a.bind("change",c.proxy(this.updateCount,this));return this.ui=this.editor.uiButton({title:f("Remaining characters before the recommended character limit is reached"),
label:f("Initializing"),text:true,icon:"ui-icon-dashboard",click:function(){this.showStatistics()}})},updateCount:function(){var a=this.options.length-c("<div></div>").html(this.editor.getCleanHtml()).text().length,b=this.ui.button,d=null,d=a>=0?f("{{charactersRemaining}} characters remaining",{charactersRemaining:a}):f("{{charactersRemaining}} characters over limit",{charactersRemaining:a*-1});b.button("option","label",d);b.button("option","text",true);a<0?b.addClass("ui-state-error"):a>=0&&a<=15?
b.addClass("ui-state-highlight").removeClass("ui-state-error"):b.removeClass("ui-state-highlight ui-state-error")},showStatistics:function(){this.processTemplate().dialog({modal:true,resizable:false,title:f("Content Statistics"),dialogClass:this.editor.options.dialogClass+" "+this.editor.options.baseClass,show:this.editor.options.dialogShowAnimation,hide:this.editor.options.dialogHideAnimation,buttons:[{text:f("OK"),click:function(){c(this).dialog("close")}}],open:function(){c(this).parent().find(".ui-dialog-buttonpane").find("button:eq(0)").button({icons:{primary:"ui-icon-circle-check"}})},
close:function(){c(this).dialog("destroy").remove()}})},processTemplate:function(){var a=c("<div></div>").html(this.editor.getCleanHtml()).text(),b=null,d=this.options.length-a.length,b=d<0?f("Content contains more than {{limit}} characters and may be truncated",{limit:this.options.length}):f("Content will not be truncated"),e=null,e=a.split(" ").length,e=e==1?f("{{words}} word",{words:e}):f("{{words}} words",{words:e}),g=null,g=a.split(". ").length,g=f("{{sentences}} sentences",{sentences:g}),h=
null,h=d>=0?f("{{characters}} characters, {{charactersRemaining}} remaining",{characters:a.length,charactersRemaining:d}):f("{{characters}} characters, {{charactersRemaining}} over the recommended limit",{characters:a.length,charactersRemaining:d*-1});return c(this.editor.getTemplate("length.dialog",{characters:h,words:e,sentences:g,truncation:b}))}}});c.ui.editor.registerPlugin("link",{visible:null,dialog:null,types:{},defaultLinkTypes:[{type:"external",title:f("Page on this or another website"),
...
this.plugin.editor.showWarning(f("The url for the file you inserted doesn't look well formed"));return b},validate:function(a){var b="."+this.options.baseClass+"-error-message-file-url",d=true;this.options.regexLink.test(a.find('input[name="location"]').val())?a.find(b).has(":visible").hide():(a.find(b).size()||a.find('input[name="location"]').after(this.plugin.editor.getTemplate("link.error",c.extend({},this.options,{messageClass:this.options.baseClass+"-error-message-file-url",message:f("The URL does not look well formed")}))),
a.find(b).not(":visible").show(),d=false);return d}}],init:function(a,b){this.options=c.extend({},{panelAnimation:"fade",replaceTypes:false,customTypes:[],typeDataName:"uiWidgetEditorLinkType",dialogWidth:750,dialogHeight:"auto",dialogMinWidth:670,regexLink:/^(http|https|ftp):\/\/[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(:[0-9]{1,5})?(\/.*)?$/i,regexEmail:/[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/},b);a.bind("save",
this.repairLinks,this);a.bind("cancel",this.cancel,this)},initTypes:function(){this.types={};for(var a={type:null,title:null,content:null,plugin:this,options:this.options,attributes:function(){},init:function(){return this},show:function(){},editing:function(a){if(a.attr("class"))for(var b=this.classes.split(/\s/gi),d=0;d<b.length;d++)if(b[d].trim()&&c(a).hasClass(b[d]))return true;return false},focusSelector:null,focus:function(){if(this.focusSelector){var a=c(this.focusSelector),b=a.val();a.val("");
a.focus().val(b)}}},b=null,b=this.options.replaceTypes?this.options.customTypes:c.merge(this.defaultLinkTypes,this.options.customTypes),d,e=0;e<b.length;e++)d=c.extend({},a,b[e],{classes:this.options.baseClass+"-"+b[e].type}).init(),this.types[d.type]=d},show:function(){if(!this.visible){this.selectedElement=this.editor.getSelectedElements().first();var a=this.selectedElement.is("a"),b=this.options,d=i.saveSelection(),e=this,g=this.dialog=c(this.editor.getTemplate("link.dialog",b)).appendTo("body");
this.initTypes();var h=this.dialog.find("fieldset"),j;for(j in this.types)c(this.editor.getTemplate("link.label",this.types[j])).appendTo(h);h.find('input[type="radio"]').bind("change."+this.editor.widgetName,function(){e.typeChange(e.types[c(this).val()],a)});g.dialog({autoOpen:false,modal:true,resizable:true,width:b.dialogWidth,minWidth:b.dialogMinWidth,height:b.dialogHeight,title:a?f("Edit Link"):f("Insert Link"),dialogClass:b.baseClass+" "+b.dialogClass,buttons:[{text:a?f("Update Link"):f("Insert Link"),
click:function(){i.restoreSelection(d);e.apply(a)?c(this).dialog("close"):d=i.saveSelection()}},{text:f("Cancel"),click:function(){i.restoreSelection(d);c(this).dialog("close")}}],beforeopen:function(){e.dialog.find("."+e.options.baseClass+"-content").hide()},open:function(){e.visible=true;var b=g.parent().find(".ui-dialog-buttonpane");b.find("button:eq(0)").button({icons:{primary:"ui-icon-circle-check"}});b.find("button:eq(1)").button({icons:{primary:"ui-icon-circle-close"}});var d=g.find('.ui-editor-link-menu input[type="radio"]');
d.first().attr("checked","checked");var f=false;if(a)for(var h in e.types)if(f=e.types[h].editing(e.selectedElement)){d.filter('[value="'+h+'"]').attr("checked","checked");e.typeChange(e.types[h],a);break}(!a||a&&!f)&&e.typeChange(e.types[d.filter(":checked").val()],a);c(this).unbind("keyup."+e.editor.widgetName).bind("keyup."+e.editor.widgetName,function(a){a.keyCode==13&&(a=e.types[d.filter(":checked").val()],(!c.isFunction(a.validate)||a.validate(e.dialog.find("."+e.options.baseClass+"-content")))&&
b.find("button:eq(0)").trigger("click"))})},close:function(){e.visible=false;g.find("."+b.baseClass+"-content").hide();c(this).dialog("destroy")}}).dialog("open")}},apply:function(a){var b=this.types[this.dialog.find('input[type="radio"]:checked').val()],d=b.attributes(this.dialog.find("."+this.options.baseClass+"-content"),a);if(!d)return true;var e=this.editor.outerHtml(c("<a>"+(d.title?d.title:d.href)+"</a>").attr(c.extend({},d,{target:"_blank"})));a?(this.selectedElement[0].className=this.selectedElement[0].className.replace(RegExp(this.options.baseClass+
"-[a-zA-Z]+","g"),""),this.selectedElement.addClass(b.classes).attr(d),this.editor.showConfirm(f("Updated link: {{link}}",{link:e}))):(this.editor.wrapTagWithAttribute("a",c.extend(d,{id:this.editor.getUniqueId()}),b.classes),this.editor.showConfirm(f("Added link: {{link}}",{link:e})),this.selectedElement=c("#"+d.id).removeAttr("id"));this.selectedElement.data(this.options.baseClass+"-href",d.href);return true},typeChange:function(a,b){var d=this.dialog.find("."+this.options.baseClass+"-content"),
e=d.closest("."+this.options.baseClass+"-wrap"),f=a.ajaxUri&&!this.types[a.type].content;f&&e.addClass(this.options.baseClass+"-loading");var h=this;d.hide(this.options.panelAnimation,function(){f?c.ajax({url:a.ajaxUri,type:"get",success:function(f){d.html(f);h.types[a.type].content=f;e.removeClass(h.options.baseClass+"-loading");a.show(d,b);d.show(h.options.panelAnimation,c.proxy(a.focus,a))}}):(d.html(a.content),a.show(d,b),d.show(h.options.panelAnimation,c.proxy(a.focus,a)))})},remove:function(){this.editor.unwrapParentTag("a")},
repairLinks:function(){var a=this;this.editor.getElement().find('a[class^="'+this.options.baseClass+'"]').each(function(){c(this).data(a.options.baseClass+"-href")&&c(this).attr("href",c(this).data(a.options.baseClass+"-href"))})},cancel:function(){this.dialog&&c(this.dialog.dialog("close"))}});c.ui.editor.registerUi({link:{init:function(a){a.bind("selectionChange",this.change,this);return a.uiButton({title:f("Insert Link"),click:function(){a.getPlugin("link").show()}})},change:function(){this.editor.getSelectedElements().length?
this.ui.enable():this.ui.disable()}},unlink:{init:function(a){a.bind("selectionChange",this.change,this);a.bind("show",this.change,this);return a.uiButton({title:f("Remove Link"),click:function(){a.getPlugin("link").remove()}})},change:function(){this.editor.getSelectedElements().is("a")?this.ui.enable():this.ui.disable()}}});c.ui.editor.registerUi({listUnordered:{init:function(a){return a.uiButton({title:f("Unordered List"),click:function(){a.selectionExists(i.getSelection())?(a.toggleWrapper("ul"),
a.toggleWrapper("li")):a.insertElement("<ul><li>First list item</li></ul>")}})}},listOrdered:{init:function(a){return a.uiButton({title:f("Ordered List"),click:function(){a.selectionExists(i.getSelection())?(a.toggleWrapper("ol"),a.toggleWrapper("li")):a.insertElement("<ol><li>First list item</li></ol>")}})}}});c.ui.editor.registerUi({logo:{ui:null,init:function(){return this.ui=this.editor.uiButton({title:f("Learn More About the Raptor WYSIWYG Editor"),click:function(){k.open("http://www.jquery-raptor.com/about/editors/",
"_blank")},ready:function(){var a=function(b){var d=typeof b;if(d!="object"||b===null)return d=="string"&&(b='"'+b+'"'),String(b);else{var e=[],f=b&&c.isArray(b);c.each(b,function(b,c){d=typeof c;d=="string"?c='"'+c+'"':d=="object"&c!==null&&(c=a(c));e.push((f?"":'"'+b+'":')+String(c))});return(f?"[":"{")+String(e)+(f?"]":"}")}},b={enableUi:this.options.enableUi,enablePlugins:this.options.enablePlugins,disabledPlugins:a(this.options.disabledPlugins),ui:a(this.options.ui),t:(new Date).getTime()},d=
[],e;for(e in b)d.push(e+"="+encodeURIComponent(b[e]));this.ui.button.find(".ui-button-icon-primary").css({"background-image":"url(http://www.jquery-raptor.com/logo/0.0.1?"+d.join("&")+")"})}})}}});c.ui.editor.registerPlugin("paste",{init:function(a,b){var d=false,e=false,g=this;a.getElement().bind("paste."+a.widgetName,c.proxy(function(){if(d)return false;d=true;var h=i.saveSelection();c(".uiWidgetEditorPasteBin").length&&c(".uiWidgetEditorPasteBin").remove();c('<div class="uiWidgetEditorPasteBin" contenteditable="true" style="width: 1px; height: 1px; overflow: hidden; position: fixed; top: -1px;" />').appendTo("body");
c(".uiWidgetEditorPasteBin").focus();k.setTimeout(function(){var j=c(".uiWidgetEditorPasteBin").html(),j=g.filterAttributes(j),j=g.filterChars(j),j={html:j,plain:c("<div/>").html(j).text(),markup:g.stripAttributes(j)};e=c(a.getTemplate("paste.dialog",j));e.find(".ui-editor-paste-area").bind("keyup."+a.widgetname,function(){g.updateAreas(this,e)});c(e).dialog({modal:true,width:650,height:500,resizable:true,title:"Paste",position:"center",show:b.dialogShowAnimation,hide:b.dialogHideAnimation,dialogClass:b.baseClass+
" "+b.dialogClass,buttons:[{text:f("Insert"),click:function(){var b=null,b=c(this).find(".ui-editor-paste-area:visible"),b=b.hasClass("ui-editor-paste-plain")||b.hasClass("ui-editor-paste-source")?b.val():b.html(),b=g.filterAttributes(b),b=g.filterChars(b);i.restoreSelection(h);a.replaceSelection(b);d=false;c(this).dialog("close")}},{text:f("Cancel"),click:function(){i.restoreSelection(h);d=false;c(this).dialog("close")}}],open:function(){var a=c(this).find(".ui-editor-paste-panel-tabs");a.find("ul li").click(function(){a.find("ul li").removeClass("ui-state-active").removeClass("ui-tabs-selected");
c(this).addClass("ui-state-active").addClass("ui-tabs-selected");a.children("div").hide().eq(c(this).index()).show()});var b=e.parent().find(".ui-dialog-buttonpane");b.find("button:eq(0)").button({icons:{primary:"ui-icon-circle-check"}});b.find("button:eq(1)").button({icons:{primary:"ui-icon-circle-close"}})},close:function(){d=false;c(this).dialog("destroy").remove()}});c(".uiWidgetEditorPasteBin").remove()},0);return true},this))},filterAttributes:function(a){c.each([{regexp:/(<meta\s*[^>]*\s*>)|(<\s*link\s* href="file:[^>]*\s*>)|(<\/?\s*\w+:[^>]*\s*>)/gi,
...
" ")},stripAttributes:function(a){a=c("<div/>").html(a);c(a.find("*")).each(function(){var a=c.map(this.attributes,function(a){return a.name}),d=c(this);c.each(a,function(a,b){d.removeAttr(b)})});return a.html()},updateAreas:function(a,b){var d=b.find('.ui-editor-paste-synchronize-text input[type="checkbox"]'),e=c(a).is("textarea")?c(a).val():c(a).html();d.attr("checked")&&(c(a).hasClass("ui-editor-paste-plain")||b.find(".ui-editor-paste-plain").val(c("<div/>").html(e).text()),c(a).hasClass("ui-editor-paste-rich")||
b.find(".ui-editor-paste-rich").html(e),c(a).hasClass("ui-editor-paste-source")||b.find(".ui-editor-paste-source").html(e),c(a).hasClass("ui-editor-paste-markup")||b.find(".ui-editor-paste-markup").html(this.stripAttributes(e)))}});c.ui.editor.registerPlugin("placeholder",{init:function(a,b){b=c.extend({},{content:"[Your content here]",tag:"p",select:true},b);this.show=function(){if(!c.trim(a.getElement().html())){var d=c(document.createElement(b.tag)).html(b.content);a.getElement().html(d);b.select&&
a.selectInner(d)}};a.bind("show",this.show)}});c.ui.editor.registerUi({raptorize:{init:function(a){var b=a.uiButton({title:f("Raptorize"),ready:function(){b.button.raptorize&&b.button.raptorize()}});return b}}});c.ui.editor.registerPlugin("saveJson",{options:{id:{attr:"name"},postName:"content",showResponse:false,appendId:false,ajax:{url:"/",type:"post",cache:false}},init:function(){},getId:function(){if(typeof this.options.id==="string")return this.options.id;else if(typeof this.options.id==="function")return this.options.id.apply(this,
[this.editor.getOriginalElement()]);else if(this.options.id.attr){var a=this.editor.getOriginalElement().attr(this.options.id.attr);if(a)return a}return null},getData:function(){var a={};a[this.getId()]=this.editor.save();return a},save:function(){this.message=this.editor.showLoading(f("Saving changes..."));var a={},b=0;this.editor.unify(function(d){d.isDirty()&&(b++,d=d.getPlugin("saveJson"),c.extend(a,d.getData()))});this.dirty=b;this.requests=this.failed=this.saved=0;this.ajax(a)},done:function(a){this.options.multiple?
...
this)):this.editor.showConfirm(f("Successfully saved {{saved}} content block(s).",this),{delay:1E3,hide:function(){this.editor.unify(function(a){a.disableEditing();a.hideToolbar()})}})),this.message.hide(),this.message=null},ajax:function(a,b){var d=c.extend(true,{},this.options.ajax);if(c.isFunction(d.data))d.data=d.data.apply(this,[b,a]);else if(this.options.postName)d.data={},d.data[this.options.postName]=JSON.stringify(a);if(c.isFunction(d.url))d.url=d.url.apply(this,[b]);this.requests++;c.ajax(d).done(c.proxy(this.done,
this)).fail(c.proxy(this.fail,this)).always(c.proxy(this.always,this))}});c.ui.editor.registerUi({save:{options:{plugin:"saveJson"},init:function(a){return a.uiButton({title:f("Save"),icon:"ui-icon-save",click:function(){a.getPlugin(this.options.plugin).save()}})}}});c.ui.editor.registerPlugin({snippet:{ids:[],enabled:false,init:function(a,b){if(b.snippets){for(var c=0,e=b.snippets.length;c<e;c++)this.createSnippet(b.snippets[c],a);a.bind("restore",this.createButtons,this);a.bind("save",this.disable,
this);a.bind("cancel",this.disable,this);a.bind("enabled",this.enable,this);a.bind("disabled",this.disable,this)}},createSnippet:function(){},enable:function(){this.enabled=true;this.createButtons()},disable:function(){this.removeButtons();this.enabled=false},createButtons:function(){for(var a=this.editor,b=0,c=this.options.snippets.length;b<c;b++){var e=this.options.snippets[b];e.repeatable&&this.createButton(e,a)}},createButton:function(a,b){if(this.enabled){var d=this,e=b.getUniqueId();this.ids.push(e);
var f=c("<button/>").addClass(d.options.baseClass).addClass(d.options.baseClass+"-button").addClass(d.options.baseClass+"-button-"+a.name).addClass(e).text("Add").click(function(){d.insertSnippet.call(d,a,b,this)}),h=a.buttonAfter||b.getElement();c.isFunction(h)?h.call(this,f,a):f.insertAfter(h);c("."+e).button({icons:{primary:"ui-icon-plusthick"}})}},removeButtons:function(){if(this.enabled){for(var a=0,b=this.ids.length;a<b;a++)c("."+this.ids[a]).remove();for(a=0,b=this.options.snippets.length;a<
b;a++){var d=this.options.snippets[a];c.isFunction(d.clean)&&d.clean.call(d,this,this.editor)}}},insertSnippet:function(a,b,d){var e=c(a.template).html(),f=a.appendTo||b.getElement();c.isFunction(f)?f.call(this,e,a,d):c(e).appendTo(f);b.disableEditing();b.enableEditing()}}});c.ui.editor.registerUi({tagMenu:{init:function(a){a.bind("selectionChange",this.change,this);a.bind("show",this.change,this);return a.uiSelectMenu({name:"tagMenu",title:f("Change HTML tag of selected element"),select:c(a.getTemplate("tagmenu.menu")),
change:function(b){a.tagSelection(b)}})},change:function(){var a=this.editor.getSelectedElements()[0];a&&(a=a.tagName.toLowerCase(),this.ui.selectMenu.find("option[value="+a+"]").length?this.ui.val(a):this.ui.val("na"))}}});c.ui.editor.registerPlugin("toolbarTip",{init:function(){this.bind("show, tagTreeUpdated",function(){c(".ui-editor-wrapper [title]").each(function(){c(this).attr("data-title",c(this).attr("title"));c(this).removeAttr("title")})})}});(function(){var a=null,b=0;c.ui.editor.registerPlugin("unsavedEditWarning",
{options:{},init:function(d){a||(a=c(d.getTemplate("unsavededitwarning.warning",this.options)).attr("id",d.getUniqueId()).appendTo("body"));var e=this;d.bind("dirty",function(){b++;b>0&&a.addClass(e.options.baseClass+"-visible")});d.bind("cleaned",function(){b--;b===0&&a.removeClass(e.options.baseClass+"-visible")})},show:function(){},hide:function(){},reposition:function(){}})})();c.ui.editor.registerUi({viewSource:{dialog:null,init:function(a){a.bind("hide",this.hide,this);return a.uiButton({title:f("View / Edit Source"),
click:function(){this.show()}})},hide:function(){this.dialog&&c(this.dialog).dialog("destroy").remove();this.dialog=null;c(this.ui.button).button("option","disabled",false)},show:function(){if(!this.dialog){c(this.ui.button).button("option","disabled",true);var a=this;this.dialog=c(this.editor.getTemplate("viewsource.dialog"));this.dialog.dialog({modal:false,width:600,height:400,resizable:true,title:f("View Source"),autoOpen:true,dialogClass:a.options.baseClass+" "+a.options.dialogClass,buttons:[{text:f("Apply Source"),
click:function(){a.editor.setHtml(c(this).find("textarea").val())}},{text:f("Close"),click:function(){a.hide()}}],open:function(){var b=c(this).parent().find(".ui-dialog-buttonpane");b.find("button:eq(0)").button({icons:{primary:"ui-icon-circle-check"}});b.find("button:eq(1)").button({icons:{primary:"ui-icon-circle-close"}});c(this).find("textarea").val(c.trim(a.editor.getHtml()))},close:function(){a.hide()}})}}}})})(jQuery,window,rangy);
