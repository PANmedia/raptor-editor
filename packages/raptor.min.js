...
this.history=[];this.present=0;this.historyEnabled=true;if(r(this)){this.cloneDomTools();this.setOriginalHtml(this.element.is(":input")?this.element.val():this.element.html());if(this.options.replace)this.replaceOriginal(),this.options.replace=false;this.attach();this.loadPlugins();this.dirty=false;this.previousSelection=this.previousContent=null;this.ready=true;this.fire("ready");this.options.autoEnable&&c(function(){a.enableEditing();a.showToolbar()})}},attach:function(){this.bind("change",this.historyPush);
this.bind("selectionChange",this.updateTagTree);this.bind("show",this.updateTagTree);var a=c.proxy(this.checkChange,this);this.getElement().find("img").bind("click."+this.widgetName,c.proxy(function(a){this.selectOuter(a.target)},this));this.getElement().bind("mouseup."+this.widgetName,a);this.getElement().bind("keyup."+this.widgetName,a);c(k).bind("beforeunload",c.proxy(c.ui.editor.unloadWarning,c.ui.editor))},reinit:function(){if(this.ready){var a=this.enabled,b=this.visible;this.destruct();this._init();
...
d.options,{baseClass:d.options.baseClass+g+"-select-menu"},e.options,d.options.ui[a]);if(!e.title)e.title=f("Unnamed Select Menu");e.wrapper=c('<div class="ui-editor-selectmenu-wrapper"/>').append(e.select.hide());e.selectMenu=c('<div class="ui-selectmenu ui-editor-selectmenu"/>').appendTo(e.wrapper);e.menu=c('<div class="ui-selectmenu-menu ui-editor-selectmenu-menu ui-widget-content ui-corner-bottom ui-corner-tr"/>').appendTo(e.wrapper);e.select.find("option").each(function(){c("<div/>").addClass("ui-selectmenu-menu-item ui-editor-selectmenu-menu-item").addClass("ui-corner-all").html(c(this).html()).appendTo(e.menu).bind("mouseenter."+
