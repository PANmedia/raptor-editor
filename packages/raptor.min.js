...
this.timer=k.setTimeout(function(){this.timer=null;e.hide()},d.delay,this)},hide:function(){if(this.timer)k.clearTimeout(this.timer),this.timer=null;this.element.stop().slideUp(c.proxy(function(){c.isFunction(d.hide)&&d.hide.call(this);this.element.remove()},this))}};e.element=c(this.getTemplate("message",{type:a,message:b})).hide().appendTo(this.messages).find(".ui-editor-message-close").click(function(){e.hide()}).end();e.show();return e},showLoading:function(a,b){return this.showMessage("clock",
...
e.button=c("<div/>").addClass("ui-selectmenu-button ui-editor-selectmenu-button ui-button ui-state-default").attr("title",e.title).append(g).append(h).prependTo(e.selectMenu);e.button.bind("click."+d.widgetName,function(){c(".ui-editor-selectmenu-visible").removeClass("ui-editor-selectmenu-visible");e.menu.css("min-width",e.button.outerWidth()+10);e.wrapper.toggleClass("ui-editor-selectmenu-visible");return false});g=e.select.find("option[value="+e.select.val()+"]").html();e.button.find(".ui-selectmenu-text").html(g);
return e.wrapper},update:function(){var a=this.select.find("option[value="+this.select.val()+"]").html();this.button.find(".ui-selectmenu-text").html(a)},val:function(){var a=this.select.val.apply(this.select,arguments);this.update();return a},change:function(){}},a)},getPlugin:function(a){return this.plugins[a]},loadPlugins:function(){if(!this.options.plugins)this.options.plugins={};for(var a in c.ui.editor.plugins)if(!(this.options.enablePlugins===false&&typeof this.options.plugins[a]==="undefined"||
this.options.plugins[a]===false)&&c.inArray(a,this.options.disabledPlugins)===-1){var b=c.extend({},c.ui.editor.plugins[a]),d=a.replace(/([A-Z])/g,function(a){return"-"+a.toLowerCase()}),d=c.extend(true,{},this.options,{baseClass:this.options.baseClass+"-"+d},b.options,this.options.plugins[a]);b.editor=this;b.options=d;b.init(this,d);this.plugins[a]=b}},isDirty:function(){return this.dirty},getHtml:function(){var a=this.getElement().html(),a=c("<div/>").html(a);a.find(".rangySelectionBoundary").remove();
...
return a},setOriginalHtml:function(a){this.originalHtml=a},bind:function(a,b,d){var e=this.events;c.each(a.split(","),function(a,f){f=c.trim(f);e[f]||(e[f]=[]);e[f].push({context:d,callback:b})})},unbind:function(a,b,d){for(var c=0,g=this.events[a].length;c<g;c++)this.events[a][c]&&this.events[a][c].callback===b&&this.events[a][c].context===d&&this.events[a].splice(c,1)},fire:function(a,b,d){d||this.fire("before:"+a,b,true);if(this.events[a])for(var e=0,g=this.events[a].length;e<g;e++){var f=this.events[a][e];
f.callback.call(f.context||this)}b!==false&&c.ui.editor.fire(a);d||this.fire("after:"+a,b,true)}});c.extend(c.ui.editor,{defaults:{plugins:{},ui:{},bind:{},domTools:{savedSelection:false,saveSelection:function(){this.savedSelection=i.saveSelection()},restoreSelection:function(){if(this.savedSelection)i.restoreSelection(this.savedSelection),this.savedSelection=false},eachRange:function(a,b,d){for(var b=b||i.getSelection(),d=d||this,c,g=0;c=b.getAllRanges()[g++];)a.call(d,c)},constrainSelection:function(a,
b){var a=c(a)[0],b=b||i.getSelection(),d;c(b.getAllRanges()).each(function(e,g){d=this.commonAncestorContainer.nodeType===3?c(g.commonAncestorContainer).parent()[0]:g.commonAncestorContainer;a!==d&&!c.contains(a,d)&&b.removeRange(g)})},getSelectedElements:function(){var a=new jQuery;this.eachRange(function(b){a.push(this.getSelectedElement(b)[0])});return a},getSelectedElement:function(a){return c(a.commonAncestorContainer.nodeType===3?a.commonAncestorContainer.parentNode:a.commonAncestorContainer)},
unwrapParentTag:function(a){this.getSelectedElements().each(function(){c(this).is(a)&&c(this).replaceWith(c(this).html())})},wrapTagWithAttribute:function(a,b,d){this.eachRange(function(c){c=this.getSelectedElement(c);c.is(a)?c.attr(b):this.toggleWrapper(a,{classes:d,attributes:b})})},selectInner:function(a,b){b=b||i.getSelection();b.removeAllRanges();c(a).focus().contents().each(function(){var a=i.createRange();a.selectNodeContents(this);b.addRange(a)})},selectOuter:function(a,b){b=b||i.getSelection();
b.removeAllRanges();c(a).each(function(){var a=i.createRange();a.selectNode(this);b.addRange(a)}).focus()},toggleWrapper:function(a,b){var b=b||{},d=i.createCssClassApplier(b.classes||"",{normalize:true,elementTagName:a,elementProperties:b.attributes||{}});this.eachRange(function(e){this.rangeEmptyTag(e)?this.replaceRange(c("<"+a+"/>").addClass(b.classes).attr(b.attributes||{}).append(this.domFragmentToHtml(e.cloneContents())),e):d.toggleRange(e)})},rangeEmptyTag:function(a){a=this.domFragmentToHtml(a.cloneContents());
typeof a==="string"&&(a=a.replace(/([ #;&,.+*~\':"!^$[\]()=>|\/@])/g,"\\$1"));return c(a).is(":empty")?true:false},execCommand:function(a,b,c){document.execCommand(a,b,c)},insertTag:function(a,b){this.eachRange(function(b){b.insertNode(c("<"+a+"/>")[0])},b)},insertTagAtEnd:function(a,b){this.eachRange(function(b){b.insertNodeAtEnd(c("<"+a+"/>")[0])},b)},insertElement:function(a,b,d){this.eachRange(function(d){c(a).each(function(){d.insertNode(b===false?this:this.cloneNode(true))})},d)},insertElementAtEnd:function(a,
b,d){this.eachRange(function(d){c(a).each(function(){d.insertNodeAtEnd(b===false?this:this.cloneNode(true))})},d)},toggleBlockStyle:function(a,b){this.eachRange(function(d){for(d=c(d.commonAncestorContainer);d.length&&d[0]!==b[0]&&(d[0].nodeType===3||d.css("display")==="inline");)d=d.parent();d[0]===b[0]&&b.css("display")!=="inline"&&(this.wrapInner(b,"div"),d=b.children().first());this.toggleStyle(d,a)})},wrapInner:function(a,b){this.saveSelection();c(a).each(function(){var d=c("<"+b+"/>").html(c(this).html());
a.html(d)});this.restoreSelection()},wrapRange:function(a){a.replaceContents()},inverseWrapWithTagClass:function(a,b,d,e){this.saveSelection();var g="domTools"+Math.ceil(Math.random()*1E7);this.eachRange(function(a){var c=i.createCssClassApplier(e,{elementTagName:d});c.isAppliedToRange(a)?c.toggleSelection():i.createCssClassApplier(b,{elementTagName:g}).toggleSelection()});c(g).each(function(){c(this).replaceWith(c("<"+a+"/>").addClass(b).html(c(this).html()))});this.restoreSelection()},toggleStyle:function(a,
b){c.each(b,function(b,e){c(a).css(b)===e?c(a).css(b,""):c(a).css(b,e)})},getStyles:function(a){for(var b={},a=k.getComputedStyle(a[0],null),c=0;c<a.length;c++)b[a.item(c)]=a.getPropertyValue(a.item(c));return b},swapStyles:function(a,b,c){for(var e in c)a.css(e,b.css(e)),b.css(e,c[e])},replaceSelection:function(a,b){this.eachRange(function(b){this.replaceRange(a,b)},b)},replaceRange:function(a,b){var d=c("<div></div>").append(a)[0].childNodes;b.deleteContents();d.length===p||d.length===1?b.insertNode(d[0].cloneNode(true)):
c.each(d,function(a,c){b.insertNodeAtEnd(c.cloneNode(true))})},insertDomFragmentBefore:function(a,b,d){for(var e=0,g=a.childNodes.length;e<g;e++){var f=a.childNodes.item(e);(f=f.nodeType===3?f.nodeValue:c(f).html())&&c("<"+d+"/>").html(c.trim(f)).insertBefore(b)}},domFragmentToHtml:function(a,b){for(var d="",e=0,g=a.childNodes.length;e<g;e++){var f=a.childNodes.item(e);(f=f.nodeType===3?f.nodeValue:this.outerHtml(f))&&(d+=f)}b&&(d=c("<"+b+">"+d+"</"+b+">"),d.find("p").wrapInner("<"+b+"/>"),d.find("p > *").unwrap(),
d=c("<div></div>").html(d).html());return d},selectionExists:function(a){var b=false;this.eachRange(function(a){this.isEmpty(a)||(b=true)},a);return b},isEmpty:function(a){return a.startOffset===a.endOffset&&a.startContainer===a.endContainer},expandToParent:function(a){a.setStartBefore(a.startContainer);a.setEndAfter(a.endContainer)},changeTag:function(a,b){this.insertDomFragmentBefore(a.extractContents(),a.startContainer,b);c(a.startContainer).remove()},tagSelection:function(a,b){this.eachRange(function(b){this.isEmpty(b)?
...
change:function(b){a.tagSelection(b)}})},change:function(){var a=this.editor.getSelectedElements()[0];a&&(a=a.tagName.toLowerCase(),this.ui.selectMenu.find("option[value="+a+"]").length?this.ui.val(a):this.ui.val("na"))}}});c.ui.editor.registerPlugin("toolbarTip",{init:function(){c.browser.msie||this.bind("show, tagTreeUpdated",function(){c(".ui-editor-wrapper [title]").each(function(){c(this).attr("data-title",c(this).attr("title"));c(this).removeAttr("title")})})}});(function(){var a=null,b=0;c.ui.editor.registerPlugin("unsavedEditWarning",
