...
"alignRight"],["textBold","textItalic","textUnderline","textStrike"],["textSuper","textSub"],["listUnordered","listOrdered"],["hr","quoteBlock"],["fontSizeInc","fontSizeDec"],["colorPickerBasic"],["clearFormatting"],["link","unlink"],["embed"],["floatLeft","floatNone","floatRight"],["tagMenu"],["i18n"],["raptorize"],["length"],["debugReinit","debugDestroy"]];this.element.attr("id")||this.element.attr("id",this.getUniqueId());this.ready=!1;this.events={};this.ui={};this.plugins={};this.templates=c.extend({},
c.ui.editor.templates);this.path=this.toolbarWrapper=this.toolbar=this.wrapper=null;this.visible=this.enabled=!1;this.uiObjects={};this.hotkeys={};this.options.enableHotkeys&&this.registerHotkey(this.hotkeys);for(var b in this.options.bind)this.bind(b,this.options.bind[b]);this.history=[];this.present=0;this.historyEnabled=!0;E(this)&&(this.cloneDomTools(),this.setOriginalHtml(this.element.is(":input")?this.element.val():this.element.html()),this.options.replace&&(this.replaceOriginal(),this.options.replace=
!1),this.attach(),this.loadPlugins(),this.dirty=!1,this.previousSelection=this.previousContent=null,this.ready=!0,this.fire("ready"),this.options.autoEnable&&c(function(){a.enableEditing();a.showToolbar()}))},attach:function(){this.bind("change",this.historyPush);this.bind("selectionChange",this.updateTagTree);this.bind("show",this.updateTagTree);var a=c.proxy(this.checkChange,this);this.getElement().find("img").bind("click."+this.widgetName,c.proxy(function(a){x(a.target)},this));this.getElement().bind("mouseup."+
this.widgetName,a);this.getElement().bind("keyup."+this.widgetName,a);c(j).bind("beforeunload",c.proxy(c.ui.editor.unloadWarning,c.ui.editor))},reinit:function(){if(this.ready){var a=this.enabled,b=this.visible;this.destruct();this._init();a&&this.enableEditing();b&&this.showToolbar()}else{var c;c=function(){this.unbind("ready",c);this.reinit()};this.bind("ready",c)}},getElement:function(){return this.target?this.target:this.element},getOriginalElement:function(){return this.element},replaceOriginal:function(){if(!this.target){for(var a=
c("<div/>").html(this.element.is(":input")?this.element.val():this.element.html()).insertBefore(this.element).attr("id",this.getUniqueId()).addClass(this.element.attr("class")),b=this.options.domTools.getStyles(this.element),d=0;d<this.options.replaceStyle.length;d++)a.css(this.options.replaceStyle[d],b[this.options.replaceStyle[d]]);this.element.hide();this.bind("change",function(){this.element.is("input, textarea")?this.element.val(this.getHtml()):this.element.html(this.getHtml())});this.target=
...
(this.enabled=!1,this.getElement().attr("contenteditable",!1).removeClass(this.options.baseClass+"-editing"),i.getSelection().removeAllRanges(),this.fire("disabled"))},isEditing:function(){return this.enabled},updateTagTree:function(){if(this.isEditing()){var a=this,b="",d=[],e=0;l(function(f){var f=f.commonAncestorContainer,h;h=3===f.nodeType?c(f).parent():c(f);f=[];for(d.push(f);h[0]&&!a.isRoot(h)&&"body"!==h[0].tagName.toLowerCase();)f.push(h),h=h.parent();f.reverse();b&&(b+=" | ");b+=this.getTemplate("root");
for(h=0;h<f.length;h++)b+=this.getTemplate("tag",{element:f[h][0].tagName.toLowerCase(),data:"["+e+","+h+"]"});e++},null,this);b||(b=this.getTemplate("root"));this.path.html(b).find("a").click(function(){var b=c(this).data("ui-editor-selection");b?(x(d[b[0]][b[1]]),a.updateTagTree()):x(a.getElement())});this.fire("tagTreeUpdated")}},isRoot:function(a){return this.getElement()[0]===c(a)[0]},unify:function(a,b){!1!==b&&a(this);if(this.options.unify)for(var d=c.ui.editor.getInstances(),e=0;e<d.length;e++)d[e]!==
this&&d[e].options.unify&&a(d[e])},getUniqueId:function(){return c.ui.editor.getUniqueId()},loadMessages:function(){this.messages=c(this.getTemplate("messages")).appendTo(this.wrapper)},showMessage:function(a,b,d){var d=c.extend({},this.options.message,d),e;e={timer:null,editor:this,show:function(){this.element.slideDown();this.timer=j.setTimeout(function(){this.timer=null;e.hide()},d.delay,this)},hide:function(){this.timer&&(j.clearTimeout(this.timer),this.timer=null);this.element.stop().slideUp(c.proxy(function(){c.isFunction(d.hide)&&
...
else{var b=j.getSelection();b.removeAllRanges();b.addRange(a)}var d=this;c(function(){d.fire("show");d.getElement().focus()})}},hideToolbar:function(){this.visible&&(this.visible=!1,this.wrapper.hide(),this.fire("hide"),this.fire("resize"))},hideOtherToolbars:function(a){this.unify(function(b){b.hideToolbar(a)},!1)},isVisible:function(){return this.visible},getTemplate:function(a,b){var d;d=this.templates[a]?this.templates[a]:c.ui.editor.getTemplate(a,this.options.urlPrefix);d=d.replace(/_\(['"]{1}(.*?)['"]{1}\)/g,
function(a,b){b=b.replace(/\\(.?)/g,function(a,b){switch(b){case "\\":return"\\";case "0":return"\x00";case "":return"";default:return b}});return g(b)});b=c.extend({},this.options,b||{});b=this.getTemplateVars(b);return d=d.replace(/\{\{(.*?)\}\}/g,function(a,c){return b[c]})},getTemplateVars:function(a,b,c){b=b?b+".":"";c||(c=1);var e={},f;for(f in a)if("object"===typeof a[f]&&5>c){var h=this.getTemplateVars(a[f],b+f,++c),g;for(g in h)e[g]=h[g]}else e[b+f]=a[f];return e},historyPush:function(){this.historyEnabled&&
...
e.wrapper=c('<div class="ui-editor-selectmenu-wrapper"/>').append(e.select.hide()).addClass(e.select.attr("class"));e.selectMenu=c('<div class="ui-editor-selectmenu"/>').appendTo(e.wrapper);e.menu=c('<div class="ui-editor-selectmenu-menu ui-widget-content ui-corner-bottom ui-corner-tr"/>').appendTo(e.wrapper);e.select.find("option, .ui-editor-selectmenu-option").each(function(){c("<div/>").addClass("ui-editor-selectmenu-menu-item").addClass("ui-corner-all").html(c(this).html()).appendTo(e.menu).bind("mouseenter."+
...
b){a.children().each(function(){var a=c.map(this.attributes,function(a){if(-1===c.inArray(a.name,b))return a.name}),e=c(this);c.each(a,function(a,b){e.removeAttr(b)});element.removeAttributes(c(this),b)});return a},elementBringToTop:K,elementOuterHtml:q,elementOuterText:function(a){return a.clone().wrap("<div/>").parent().text()},elementIsBlock:function(a){return"block"===G(a.tagName)},elementDefaultDisplay:G,elementIsValid:H,fragmentToHtml:v,fragmentInsertBefore:function(a,b,d){for(var e=0,f=a.childNodes.length;e<
f;e++){var h=a.childNodes.item(e);(h=3===h.nodeType?h.nodeValue:c(h).html())&&c("<"+d+"/>").html(c.trim(h)).insertBefore(b)}},rangeExpandToParent:function(a){a.setStartBefore(a.startContainer);a.setEndAfter(a.endContainer)},rangeGetCommonAncestor:T,rangeIsEmpty:U,selectionSave:o,selectionRestore:m,selectionDestroy:I,selectionEachRange:l,selectionReplace:p,selectionSelectInner:C,selectionSelectOuter:x,selectionSelectEdge:L,selectionSelectEnd:V,selectionSelectStart:y,selectionGetHtml:z,selectionGetElements:k,
selectionToggleWrapper:r,selectionExists:function(a){var b=!1;l(function(a){U(a)||(b=!0)},a,this);return b},selectionReplaceSplittingSelectedElement:O,selectionReplaceWithinValidTags:D,stringStripTags:J,defaults:{plugins:{},ui:{},bind:{},domTools:{constrainSelection:function(a,b){var a=c(a)[0],b=b||i.getSelection(),d;c(b.getAllRanges()).each(function(e,f){d=3===this.commonAncestorContainer.nodeType?c(f.commonAncestorContainer).parent()[0]:f.commonAncestorContainer;a!==d&&!c.contains(a,d)&&b.removeRange(f)})},
...
b?this:this.cloneNode(!0))})},d,this)},toggleBlockStyle:function(a,b){l(function(d){for(d=c(d.commonAncestorContainer);d.length&&d[0]!==b[0]&&(3===d[0].nodeType||"inline"===d.css("display"));)d=d.parent();d[0]===b[0]&&"inline"!==b.css("display")&&(this.wrapInner(b,"div"),d=b.children().first());this.toggleStyle(d,a)},null,this)},wrapInner:function(a,b){o();c(a).each(function(){var d=c("<"+b+"/>").html(c(this).html());a.html(d)});m()},inverseWrapWithTagClass:function(a,b,d,e){o();var f="domTools"+
Math.ceil(1E7*Math.random());l(function(a){var c=i.createCssClassApplier(e,{elementTagName:d});c.isAppliedToRange(a)?c.toggleSelection():i.createCssClassApplier(b,{elementTagName:f}).toggleSelection()},null,this);c(f).each(function(){c(this).replaceWith(c("<"+a+"/>").addClass(b).html(c(this).html()))});m()},toggleStyle:function(a,b){c.each(b,function(b,e){c(a).css(b)===e?c(a).css(b,""):c(a).css(b,e)})},getStyles:function(a){for(var b={},a=j.getComputedStyle(a[0],null),c=0;c<a.length;c++)b[a.item(c)]=
...
{selector:"img",attributes:["src"]}]},init:function(a){a.bind("change",this.clean,this)},clean:function(){var a,b=this.editor;for(a=0;a<this.options.stripAttrs.length;a++)b.getElement().find("["+this.options.stripAttrs[a]+"]").removeAttr(this.options.stripAttrs[a]);for(a=0;a<this.options.stripAttrContent.length;a++)b.getElement().find("["+a+'="'+this.options.stripAttrs[a]+'"]').removeAttr(this.options.stripAttrs[a]);for(a=0;a<this.options.stripEmptyTags.length;a++)b.getElement().find(this.options.stripEmptyTags[a]).filter(function(){if(c(this).hasClass(plugin.options.supplementaryClass))return!1;
if(c(this).hasClass("rangySelectionBoundary")&&!1===(!1!==u)||""===c.trim(c(this).html()))return!0}).remove();for(a=0;a<this.options.stripEmptyAttrs.length;a++){var d=this.options.stripEmptyAttrs[a];b.getElement().find("["+this.options.stripEmptyAttrs[a]+"]").filter(function(){return""===c.trim(c(this).attr(d))}).removeAttr(this.options.stripEmptyAttrs[a])}var e=j.location.protocol+"//"+j.location.host,f="//"+j.location.host;for(a=0;a<this.options.stripDomains.length;a++){var h=this.options.stripDomains[a];
this.editor.getElement().find(h.selector).each(function(){for(var a=0;a<h.attributes.length;a++){var b=c(this).attr(h.attributes[a]);"undefined"!==typeof b&&(0===b.indexOf(e)?c(this).attr(h.attributes[a],b.substr(e.length)):0===b.indexOf(f)&&c(this).attr(h.attributes[a],b.substr(f.length)))}})}this.editor.getElement().find("ul, ol").each(function(){c(this).find(" > :not(li)").each(function(){G(c(this).attr("tag"))?c(this).replaceWith(c("<li>"+c(this).html()+"</li>").appendTo("body")):c(this).wrap(c("<li>"))})})}});
c.ui.editor.registerUi({clean:{init:function(a){return a.uiButton({title:g("Remove unnecessary markup from editor content"),click:function(){a.getPlugin("clean").clean()}})}}});c.ui.editor.registerUi({clearFormatting:{init:function(a){return this.editor.uiButton({title:g("Clear Formatting"),click:function(){var b=i.getSelection();if(0<b.rangeCount){var d=b.getRangeAt(0).cloneRange(),e=d.extractContents();""==v(e)&&(a.expandToParent(d),b.setSingleRange(d),e=d.extractContents());e=c("<div/>").append(v(e)).text();
...
c(this.editor.getTemplate("embed.dialog"));this.dialog.dialog({modal:!1,width:600,height:400,resizable:!0,title:g("Paste Embed Code"),autoOpen:!0,dialogClass:a.options.baseClass+" "+a.options.dialogClass,buttons:[{text:g("Embed Object"),click:function(){m();p(c(this).find("textarea").val());c(this).dialog("close")}},{text:g("Close"),click:function(){a.hide()}}],open:function(){var a=c(this).parent().find(".ui-dialog-buttonpane");a.find("button:eq(0)").button({icons:{primary:"ui-icon-circle-check"}});
...
b){return a.uiButton({title:"Insert image or uploaded file",icon:"ui-icon-image",click:function(){var a=this;o();null===this.fileManager&&(this.fileManager=c.ui.filemanager.create({enablePlugins:!1,plugins:{datatables:!XMod.FileManager.Permissions.insert?!1:{ajaxSource:"/filemanager/admin/datatables",insertionCallback:function(e){m();if(!e.length)return true;var f=function(){a.editor.fire("change");return true};if(e.length===1){var h=e[0];if(a.isImage(h)){p(a.createImage(h,b.cssPrefix+h.type));return f()}if(a.editor.selectionExists()){X("a",
{href:h.url,className:a.options.cssPrefix+"file "+a.options.cssPrefix+h.type});return f()}p(a.createAnchor(h,a.options.cssPrefix+"file "+a.options.cssPrefix+h.type));return f()}for(var g=[],i=0;i<e.length;i++){h=e[i];a.isImage(h)?g.push(c("<div/>").html(a.createImage(h,b.cssPrefix+h.type)).html()):g.push(c("<div/>").html(a.createAnchor(h,a.options.cssPrefix+"file "+a.options.cssPrefix+h.type)).html())}p(g.join(", "));return f()}},plupload:!XMod.FileManager.Permissions.upload?!1:{url:"/filemanager/admin/plupload"}}}));
c(this.fileManager).filemanager("show")}})},isImage:function(a){return-1!==c.inArray(a.type.toLowerCase(),this.imageTypes)?!0:!1},createImage:function(a,b){return c("<img/>").attr({src:a.url,title:a.name,"class":b})},createAnchor:function(a,b){return c("<a/>").attr({href:a.url,title:a.name,"class":b}).html(a.name)}}})});c.ui.editor.registerUi({floatLeft:{init:function(a){return a.uiButton({title:g("Float Image Left"),click:function(){l(function(a){c(a.commonAncestorContainer).find("img").css("float",
"left")})}})}},floatRight:{init:function(a){return a.uiButton({title:g("Float Image Right"),click:function(){l(function(a){c(a.commonAncestorContainer).find("img").css("float","right")})}})}},floatNone:{init:function(a){return a.uiButton({title:g("Remove Image Float"),click:function(){l(function(a){c(a.commonAncestorContainer).find("img").css("float","none")})}})}}});c.ui.editor.registerUi({fontSizeInc:{init:function(a,b){return a.uiButton({title:g("Increase Font Size"),click:function(){a.inverseWrapWithTagClass("big",
...
init:function(a){a.bind("change",this.change,this);return this.ui=a.uiButton({title:g("Step Forward"),click:function(){a.historyForward()}})},change:function(){this.editor.present===this.editor.history.length-1?this.ui.disable():this.ui.enable()}}});c.ui.editor.registerUi({hr:{init:function(a){return a.uiButton({title:g("Insert Horizontal Rule"),click:function(){p("<hr/>")}})}}});c.ui.editor.registerUi({i18n:{init:function(a){var b=this,d=this.persist("locale");d&&w(d);var d=c('<select autocomplete="off" name="i18n"/>'),
e;for(e in B){var f=c('<option value="'+e+'" class="'+e+'"/>');f.html(Q[e]);t===e&&f.attr("selected","selected");d.append(f)}return a.uiSelectMenu({title:g("Change Language"),select:d,change:function(a){w(b.persist("locale",a))}})}}});s("en","English",{"A preview of your embedded object is displayed below.":"A preview of your embedded object is displayed below.","Added link: {{link}}":"Added link: {{link}}","All changes will be lost!":"All changes will be lost!","Apply Source":"Apply Source","Are you sure you want to stop editing?":"Are you sure you want to stop editing?",
...
{width:d,height:e,baseClass:this.options.baseClass}));j.dialog({modal:!0,resizable:!1,title:g("Modify Image Size"),autoOpen:!0,buttons:[{text:g("Resize Image"),click:function(){i(c(this).find(f).val());c(this).dialog("close")}},{text:g("Cancel"),click:function(){i(d);c(this).dialog("close")}}],close:function(){P.editor.checkChange();c(j).remove()},open:function(){var a=c(this).find(f),b=c(this).find(h);a.keyup(function(){b.val(Math.round(Math.abs(e/d*c(this).val())));i()});b.keyup(function(){a.val(Math.round(Math.abs(d/
e*c(this).val())));i()})}});a.preventDefault();return!1},displayToolsButtonRelativeToImage:function(a){var b=c("<button>"+g("Click to edit the image")+"</button>").appendTo("body").addClass(this.options.resizeButtonClass+" "+this.options.supplementaryClass).button({text:!1,icons:{primary:"ui-icon-tools"}});b.css({position:"absolute",left:c(a).position().left+c(a).innerWidth()-c(b).outerWidth()-10+"px",top:c(a).position().top+10+"px"}).attr("contenteditable",!1).click(c.proxy(this.manuallyResizeImage,
...
options:{length:150},init:function(a){a.bind("show",c.proxy(this.updateCount,this));a.bind("change",c.proxy(this.updateCount,this));return this.ui=this.editor.uiButton({title:g("Remaining characters before the recommended character limit is reached"),label:g("Initializing"),text:!0,icon:"ui-icon-dashboard",click:function(){this.showStatistics()}})},updateCount:function(){var a=this.options.length-c("<div/>").html(this.editor.getCleanHtml()).text().length,b=this.ui.button,d=null,d=0<=a?g("{{charactersRemaining}} characters remaining",
{charactersRemaining:a}):g("{{charactersRemaining}} characters over limit",{charactersRemaining:-1*a});b.button("option","label",d);b.button("option","text",!0);0>a?b.addClass("ui-state-error"):0<=a&&15>=a?b.addClass("ui-state-highlight").removeClass("ui-state-error"):b.removeClass("ui-state-highlight ui-state-error")},showStatistics:function(){this.processTemplate().dialog({modal:!0,resizable:!1,title:g("Content Statistics"),dialogClass:this.editor.options.dialogClass+" "+this.editor.options.baseClass,
show:this.editor.options.dialogShowAnimation,hide:this.editor.options.dialogHideAnimation,buttons:[{text:g("OK"),click:function(){c(this).dialog("close")}}],open:function(){c(this).parent().find(".ui-dialog-buttonpane").find("button:eq(0)").button({icons:{primary:"ui-icon-circle-check"}})},close:function(){c(this).dialog("destroy").remove()}})},processTemplate:function(){var a=c("<div/>").html(this.editor.getCleanHtml()).text(),b=null,d=this.options.length-a.length,b=0>d?g("Content contains more than {{limit}} characters and may be truncated",
{limit:this.options.length}):g("Content will not be truncated"),e=null,e=a.split(" ").length,e=1==e?g("{{words}} word",{words:e}):g("{{words}} words",{words:e}),f=null,f=a.split(". ").length,f=g("{{sentences}} sentences",{sentences:f}),h=null,h=0<=d?g("{{characters}} characters, {{charactersRemaining}} remaining",{characters:a.length,charactersRemaining:d}):g("{{characters}} characters, {{charactersRemaining}} over the recommended limit",{characters:a.length,charactersRemaining:-1*d});return c(this.editor.getTemplate("length.dialog",
{characters:h,words:e,sentences:f,truncation:b}))}}});c.ui.editor.registerPlugin("link",{visible:null,dialog:null,types:{},defaultLinkTypes:[{type:"external",title:g("Page on this or another website"),focusSelector:'input[name="location"]',init:function(){this.content=this.plugin.editor.getTemplate("link.external",this.options);return this},show:function(a,b){var c=this;a.find('input[name="location"]').bind("keyup",function(){c.validate(a)});b&&(a.find('input[name="location"]').val(this.plugin.selectedElement.attr("href")).trigger("keyup"),
...
dialogClass:b.baseClass+" "+b.dialogClass,buttons:[{text:a?g("Update Link"):g("Insert Link"),click:function(){m();d.apply(a)&&c(this).dialog("close")}},{text:g("Cancel"),click:function(){c(this).dialog("close")}}],beforeopen:function(){d.dialog.find("."+d.options.baseClass+"-content").hide()},open:function(){d.visible=!0;var b=e.parent().find(".ui-dialog-buttonpane");b.find("button:eq(0)").button({icons:{primary:"ui-icon-circle-check"}});b.find("button:eq(1)").button({icons:{primary:"ui-icon-circle-close"}});
...
d.options.baseClass+"-content")))&&b.find("button:eq(0)").trigger("click"))})},close:function(){m();d.visible=!1;e.find("."+b.baseClass+"-content").hide();c(this).dialog("destroy")}}).dialog("open")}},apply:function(a){var b=this.types[this.dialog.find('input[type="radio"]:checked').val()],d=b.attributes(this.dialog.find("."+this.options.baseClass+"-content"),a);if(!d)return!0;m();var e=q(c("<a>"+(d.title?d.title:d.href)+"</a>").attr(c.extend({},d,{target:"_blank"})));a?(this.selectedElement[0].className=
this.selectedElement[0].className.replace(RegExp(this.options.baseClass+"-[a-zA-Z]+","g"),""),this.selectedElement.addClass(b.classes).attr(d),this.editor.showConfirm(g("Updated link: {{link}}",{link:e}))):(X("a",c.extend(d,{id:this.editor.getUniqueId()}),b.classes),this.editor.showConfirm(g("Added link: {{link}}",{link:e})),this.selectedElement=c("#"+d.id).removeAttr("id"));this.selectedElement.data(this.options.baseClass+"-href",d.href);x(this.selectedElement);o();return!0},typeChange:function(a,
...
this.editor.fire("change")},unwrapList:function(){o();var a=[],b=[],d=M(),e=N();a.push(c(d).html());b.push(d);if(c(d)[0]!==c(e)[0]){var f=d;do f=c(f).next(),a.push(c(f).html()),b.push(f);while(c(f)[0]!==c(e)[0])}f=0===c(d).prev().length;e=0===c(e).next().length;d=c(d).parent();for(listElementsIndex=0;listElementsIndex<b.length;listElementsIndex++)c(b[listElementsIndex]).remove();for(b=0;b<a.length;b++)d.parent().is("li")||(a[b]="<p>"+a[b]+"</p>");f&&e?(d.replaceWith(a.join("")),m(),a=k()[0],x(a)):
(f?c(d).before(a.join("")):e?c(d).after(a.join("")):O(a.join("")),m(),this.editor.checkChange())},wrapList:function(a){this.editor.constrainSelection(this.editor.getElement());""===c.trim(z())&&C(k());var b=c("<div>").html(z()),d=[],e=this;c(b).contents().each(function(){var a;a="block"===G(this.tagName)?J(c(this).html(),e.validChildren):J(q(c(this)),e.validChildren);a=c("<li>"+a+"</li>");""!==c.trim(a.text())&&d.push(q(a))});b=this.options.baseClass+"-selection";a="<"+a+' class="'+b+'">'+d.join("")+
"</"+a+">";m();var f=c(k()[0]).parent(),h=this.editor.getElement()[0];f===h||k()[0]===h?p(a):D(a,this.validParents);b=c(this.editor.getElement().find("."+b).removeClass(b));C(b.find("li:first")[0]);this.editor.checkChange()},toggleButtonState:function(a,b){var d=function(a){b.button.toggleClass("ui-state-highlight",a).toggleClass("ui-state-default",!a)},e=M();if(null===e||!e.length)e=this.editor.getElement();var f=N();if(null===f||!f.length)f=this.editor.getElement();e=e[0];f=f[0];if(c(e).is(a)&&
...
f);D(d,this.options.enterValidTags);d=c("#"+f).removeAttr("id");a?b?y(d.next()):e?y(d):(y(d.next()),(a=d.prev())&&""!==c.trim(a.html())&&q(a)!==this.options.enter&&d.remove()):(y(d.next()),d.remove())},restoreSelection:!1},"return+shift":{action:function(){I();var a=this.options.baseClass+"-enter-break",b=c(breakHtml).attr("id",a).appendTo("body");this.options.shiftEnterValidTags?D(this.options.shiftEnter,this.options.shiftEnterValidTags):p(b);a=c("#"+a).removeAttr("id").next();y(a)},restoreSelection:!1}}});
...
c(".uiWidgetEditorPasteBin").focus();j.setTimeout(function(){var h=c(".uiWidgetEditorPasteBin").html(),h=f.filterAttributes(h),h=f.filterChars(h),h=f.stripEmpty(h),h=f.stripAttributes(h),h=J(h,f.options.allowedTags),h={plain:c("<div/>").html(c(".uiWidgetEditorPasteBin").html()).text(),markup:h,html:c(".uiWidgetEditorPasteBin").html()};e=c(a.getTemplate("paste.dialog",h));c(e).dialog({modal:!0,width:650,height:500,resizable:!0,title:"Paste",position:"center",show:b.dialogShowAnimation,hide:b.dialogHideAnimation,
dialogClass:b.baseClass+" "+b.dialogClass,buttons:[{text:g("Insert"),click:function(){var a=null,a=c(this).find(".ui-editor-paste-area:visible"),a=a.hasClass("ui-editor-paste-plain")||a.hasClass("ui-editor-paste-source")?a.val():a.html(),a=f.filterAttributes(a),a=f.filterChars(a);m();p(a);d=!1;c(this).dialog("close")}},{text:g("Cancel"),click:function(){m();d=!1;c(this).dialog("close")}}],open:function(){var a=c(this).find(".ui-editor-paste-panel-tabs");a.find("ul.ui-tabs-nav li").click(function(){a.find("ul.ui-tabs-nav li").removeClass("ui-state-active").removeClass("ui-tabs-selected");
...
d.widgetName,function(){c("."+e.options.baseClass+"-dirty").removeClass(e.options.baseClass+"-dirty")}));d.bind("dirty",function(){b++;0<b&&(K(a),a.addClass(e.options.baseClass+"-visible"))});d.bind("cleaned",function(){b--;0===b&&a.removeClass(e.options.baseClass+"-visible")})}})})();c.ui.editor.registerUi({viewSource:{init:function(a){return a.uiButton({title:g("View / Edit Source"),click:function(){this.show()}})},show:function(){var a=this,b=c(this.editor.getTemplate("viewsource.dialog",{baseClass:a.options.baseClass,
source:a.editor.getHtml()})),d=this.ui.button;c(d).button("option","disabled",!0);b.dialog({modal:!1,width:600,height:400,resizable:!0,title:g("View Source"),autoOpen:!0,dialogClass:a.options.baseClass+" "+a.options.dialogClass,buttons:[{text:g("Apply Source"),click:function(){var b=c(this).find("textarea").val();a.editor.setHtml(b);c(this).find("textarea").val(a.editor.getHtml())}},{text:g("Close"),click:function(){c(this).dialog("close")}}],open:function(){var a=c(this).parent().find(".ui-dialog-buttonpane");
a.find("button:eq(0)").button({icons:{primary:"ui-icon-circle-check"}});a.find("button:eq(1)").button({icons:{primary:"ui-icon-circle-close"}})},close:function(){c(this).dialog("destroy").remove();c(d).button("option","disabled",!1);a.editor.checkChange()}})}}});var u=!1})(jQuery,window,rangy);
