...
disableEditing:function(){this.enabled&&(this.enabled=!1,this.getElement().attr("contenteditable",!1).removeClass(this.options.baseClass+"-editing"),i.getSelection().removeAllRanges(),this.fire("disabled"))},isEditing:function(){return this.enabled},updateTagTree:function(){if(this.isEditing()){var a=this,b="",d=[],e=0;m(function(f){var f=f.commonAncestorContainer,g;g=3===f.nodeType?c(f).parent():c(f);f=[];for(d.push(f);g[0]&&!a.isRoot(g)&&"body"!==g[0].tagName.toLowerCase();)f.push(g),g=g.parent();
...
if(c(this).hasClass("rangySelectionBoundary")&&!1===(!1!==t)||""===c.trim(c(this).html()))return!0}).remove();for(a=0;a<this.options.stripEmptyAttrs.length;a++){var d=this.options.stripEmptyAttrs[a];b.getElement().find("["+this.options.stripEmptyAttrs[a]+"]").filter(function(){return""===c.trim(c(this).attr(d))}).removeAttr(this.options.stripEmptyAttrs[a])}var e=l.location.protocol+"//"+l.location.host,f="//"+l.location.host;for(a=0;a<this.options.stripDomains.length;a++){var g=this.options.stripDomains[a];
...
for(b=d.commonAncestorContainer;b&&b.parentNode!=a.getElement().get(0);)b=b.parentNode;if(b){d.setEndAfter(b);var f=d.extractContents();d.collapseAfter(b);d.insertNode(f);d.collapseAfter(b)}d.insertNode(document.createTextNode(e))}a.checkChange()}})}}});c.ui.editor.registerPlugin("clickButtonToEdit",{hovering:!1,button:!1,options:{button:{text:!0,icons:{primary:"ui-icon-pencil"}}},init:function(a,b){var d=this;this.selection=function(){var a;document.selection?a=document.selection.createRange():document.getSelection().rangeCount&&
(a=document.getSelection().getRangeAt(0));return a};this.show=function(){if(!a.isEditing()){a.getElement().addClass(b.baseClass+"-highlight");a.getElement().addClass(b.baseClass+"-hover");var e=d.getButton();var f=a.getElement(),f=c(f),g=Math.round(f.offset().top),h=Math.round(f.offset().left),k=Math.round(f.outerWidth()),f=Math.round(f.outerHeight()),i=c(l).scrollTop(),X=c(l).height(),j=i+X,m=Math.round(f+g);i<g&&j>m||(i>g&&(g=i),f=j<m?j-g:X-(j-m));e.css({position:"absolute",top:g+(f/2-c(e).outerHeight()/
2),left:h+k/2-c(e).outerWidth()/2});e.addClass(b.baseClass+"-visible")}};this.hide=function(e){var f=d.getButton();if(!e||!(e.relatedTarget===f.get(0)||f.get(0)===c(e.relatedTarget).parent().get(0)))a.getElement().removeClass(b.baseClass+"-highlight"),a.getElement().removeClass(b.baseClass+"-hover"),f.removeClass(b.baseClass+"-visible")};this.edit=function(){d.hide();a.isEditing()||a.enableEditing();a.isVisible()||a.showToolbar(d.selection())};this.buttonOut=function(){d.getButton().hasClass(b.baseClass+
"-visible")||(l.clearTimeout(d.timeoutId),d.timeoutId=l.setTimeout(d.hide,350))};a.getElement().addClass("ui-editor-click-button-to-edit");a.bind("ready, hide, cancel",function(){a.getElement().bind("mouseenter."+a.widgetName,d.show);a.getElement().bind("mouseleave."+a.widgetName,d.hide)});a.bind("show",function(){d.destroyButton();a.getElement().unbind("mouseenter."+a.widgetName,d.show);a.getElement().unbind("mouseleave."+a.widgetName,d.hide)})},getButton:function(){!1===this.button&&(this.button=
c(this.editor.getTemplate("clickbuttontoedit.edit-button",this.options)).appendTo("body").removeClass(this.options.baseClass+"-visible"),this.button.button(this.options.button));this.button.unbind("click."+this.editor.widgetName).bind("click."+this.editor.widgetName,this.edit);this.button.unbind("mouseleave."+this.editor.widgetName).bind("mouseleave."+this.editor.widgetName,this.buttonOut);return this.button},destroyButton:function(){this.button.button("destroy").remove();this.button=!1}});c.ui.editor.registerUi("colorPickerBasic",
{init:function(a){a.bind("selectionChange",this.change,this);a.bind("show",this.change,this);var b=this;return a.uiSelectMenu({name:"colorPickerBasic",title:h("Change the color of the selected text."),select:c(a.getTemplate("color-picker-basic.menu")),change:function(a){"automatic"===a?j().parents("."+b.options.cssPrefix+"color").andSelf().each(function(){var a=c(this),b=c(this).attr("class").match(/(cms-(.*?))( |$)/ig);c.each(b,function(b,d){d=c.trim(d);a.removeClass(d)})}):r("span",{classes:b.options.classes||
b.options.cssPrefix+"color "+b.options.cssPrefix+a})}})},change:function(){this.ui.val("automatic");var a=j()[0];if(a&&(a=c(a).closest("."+this.options.cssPrefix+"color"))){var b=a.attr("class");b&&(b=a.attr("class").replace(RegExp(this.options.cssPrefix+"color","g"),""),a=b.match(/cms-(.*?)( |$)/i)[1],this.ui.select.find(".ui-editor-selectmenu-option[value="+a+"]").length&&this.ui.val(a))}}});c.ui.editor.registerPlugin("dock",{enabled:!1,docked:!1,topSpacer:null,bottomSpacer:null,options:{docked:!1,
...
icon:a.getPlugin("dock").isDocked()?"ui-icon-pin-w":"ui-icon-pin-s",click:function(){var b=a.getPlugin("dock");b.isDocked()?b.undock():b.dock();a.unify(function(a){b.isDocked()?a.getPlugin("dock").dock():a.getPlugin("dock").undock()})}})}}});c.ui.editor.registerUi({embed:{dialog:null,init:function(a){a.bind("hide",this.hide,this);return a.uiButton({icon:"ui-icon-youtube",title:h("Embed object"),click:function(){this.show()}})},hide:function(){this.dialog&&c(this.dialog).dialog("destroy").remove();
this.dialog=null;c(this.ui.button).button("option","disabled",!1)},show:function(){if(!this.dialog){c(this.ui.button).button("option","disabled",!0);var a=this;o();this.dialog=c(this.editor.getTemplate("embed.dialog"));this.dialog.dialog({modal:!1,width:600,height:400,resizable:!0,title:h("Paste Embed Code"),autoOpen:!0,dialogClass:a.options.baseClass+" "+a.options.dialogClass,buttons:[{text:h("Embed Object"),click:function(){n();p(c(this).find("textarea").val());c(this).dialog("close")}},{text:h("Close"),
click:function(){a.hide()}}],open:function(){var a=c(this).parent().find(".ui-dialog-buttonpane");a.find("button:eq(0)").button({icons:{primary:"ui-icon-circle-check"}});a.find("button:eq(1)").button({icons:{primary:"ui-icon-circle-close"}});var d=c(this).find(".ui-editor-embed-panel-tabs");d.find("ul li").click(function(){d.find("ul li").removeClass("ui-state-active").removeClass("ui-tabs-selected");c(this).addClass("ui-state-active").addClass("ui-tabs-selected");d.children("div").hide().eq(c(this).index()).show()});
var e=c(this).find(".ui-editor-embed-preview");c(this).find("textarea").change(function(){c(e).html(c(this).val())})},close:function(){a.hide()}})}}}});c.ui.editor.registerPlugin("emptyElement",{options:{tag:"<p/>"},init:function(){this.bind("change",this.change)},change:function(){var a=this;this.textNodes(this.editor.getElement()).each(function(){c(this).wrap(c(a.options.tag));U(this)});this.editor.checkChange()},textNodes:function(a){return c(a).contents().filter(function(){return 3==this.nodeType&&
c.trim(this.nodeValue).length})}});c(function(){c.ui.editor.registerUi({insertFile:{fileManager:null,imageTypes:["jpg","jpeg","png","gif"],init:function(a,b){return a.uiButton({title:"Insert image or uploaded file",icon:"ui-icon-image",click:function(){var a=this;o();null===this.fileManager&&(this.fileManager=c.ui.filemanager.create({enablePlugins:!1,plugins:{datatables:!XMod.FileManager.Permissions.insert?!1:{ajaxSource:"/filemanager/admin/datatables",insertionCallback:function(e){n();if(!e.length)return true;
var f=function(){a.editor.fire("change");return true};if(e.length===1){var g=e[0];if(a.isImage(g)){p(a.createImage(g,b.cssPrefix+g.type));return f()}if(a.editor.selectionExists()){W("a",{href:g.url,className:a.options.cssPrefix+"file "+a.options.cssPrefix+g.type});return f()}p(a.createAnchor(g,a.options.cssPrefix+"file "+a.options.cssPrefix+g.type));return f()}for(var h=[],k=0;k<e.length;k++){g=e[k];a.isImage(g)?h.push(c("<div/>").html(a.createImage(g,b.cssPrefix+g.type)).html()):h.push(c("<div/>").html(a.createAnchor(g,
a.options.cssPrefix+"file "+a.options.cssPrefix+g.type)).html())}p(h.join(", "));return f()}},plupload:!XMod.FileManager.Permissions.upload?!1:{url:"/filemanager/admin/plupload"}}}));c(this.fileManager).filemanager("show")}})},isImage:function(a){return-1!==c.inArray(a.type.toLowerCase(),this.imageTypes)?!0:!1},createImage:function(a,b){return c("<img/>").attr({src:a.url,title:a.name,"class":b})},createAnchor:function(a,b){return c("<a/>").attr({href:a.url,title:a.name,"class":b}).html(a.name)}}})});
c.ui.editor.registerUi({floatLeft:{init:function(a){return a.uiButton({title:h("Float Image Left"),click:function(){m(function(a){c(a.commonAncestorContainer).find("img").css("float","left")})}})}},floatRight:{init:function(a){return a.uiButton({title:h("Float Image Right"),click:function(){m(function(a){c(a.commonAncestorContainer).find("img").css("float","right")})}})}},floatNone:{init:function(a){return a.uiButton({title:h("Remove Image Float"),click:function(){m(function(a){c(a.commonAncestorContainer).find("img").css("float",
