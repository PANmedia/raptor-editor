...
var f=c("#elRaptor").css({position:"fixed",bottom:"-700px",right:"0",display:"block"});if(j.enterOn=="timer")setTimeout(i,j.delayTime);else if(j.enterOn=="click")p.bind("click",function(c){c.preventDefault();n||i()});else if(j.enterOn=="konami-code"){var r=[];c(window).bind("keydown.raptorz",function(f){r.push(f.keyCode);r.toString().indexOf("38,38,40,40,37,39,37,39,66,65")>=0&&(i(),c(window).unbind("keydown.raptorz"))},true)}})}})(jQuery);
(function(c,j,i,p){function o(a,b,d){q[a]=d;t[a]=b;m||(m=a)}function n(a){m!==a&&(m=a,c.ui.editor.eachInstance(function(){this.reinit()}))}function f(a,b){m&&q[m]&&q[m][a]&&(a=q[m][a]);if(b)for(var d in b)a=a.replace("{{"+d+"}}",b[d]);return a}function r(a){if(s===p){s=true;if(u=/(iPhone|iPod|iPad).*AppleWebKit/i.test(navigator.userAgent))c("html").addClass(a.options.baseClass+"-ios"),u&&c(document).bind("scroll",function(){setInterval(function(){c("body").css("height","+=1").css("height","-=1")},
0)});c.browser.mozilla&&c("html").addClass(a.options.baseClass+"-ff");if(c.browser.msie&&c.browser.version<9){s=false;var b=c("<div></div>").addClass(a.options.baseClass+"-unsupported").html(a.getTemplate("unsupported")).appendTo("body"),d=1;b.siblings().each(function(){var a=c(this).css("z-index");!isNaN(a)&&a>d&&(d=a+1)});b.css("z-index",d);b.find("."+a.options.baseClass+"-unsupported-close").click(function(){b.remove()})}}return s}var m=null,q={},t={};c(function(){i.initialized||i.init();if(!c.isFunction(i.rangePrototype.insertNodeAtEnd))i.rangePrototype.insertNodeAtEnd=
...
this.getElement().find("img").bind("click."+this.widgetName,c.proxy(function(a){this.selectOuter(a.target)},this));this.getElement().bind("mouseup."+this.widgetName,a);this.getElement().bind("keyup."+this.widgetName,a);c(j).bind("beforeunload",c.proxy(c.ui.editor.unloadWarning,c.ui.editor))},reinit:function(){if(this.ready){var a=this.enabled,b=this.visible;this.destruct();this._init();a&&this.enableEditing();b&&this.showToolbar()}else{var d;d=function(){this.unbind("ready",d);this.reinit()};this.bind("ready",
...
this.timer=j.setTimeout(function(){this.timer=null;e.hide()},d.delay,this)},hide:function(){if(this.timer)j.clearTimeout(this.timer),this.timer=null;this.element.stop().slideUp(c.proxy(function(){c.isFunction(d.hide)&&d.hide.call(this);this.element.remove()},this))}};e.element=c(this.getTemplate("message",{type:a,message:b})).hide().appendTo(this.messages).find(".ui-editor-message-close").click(function(){e.hide()}).end();e.show();return e},showLoading:function(a,b){return this.showMessage("clock",
...
this.persist("position")||this.options.dialogPosition)||(a=[10,10]),parseInt(a[0],10)+d.outerHeight()>c(j).height()&&(a[0]=c(j).height()-d.outerHeight()),parseInt(a[1],10)+d.outerWidth()>c(j).width()&&(a[1]=c(j).width()-d.outerWidth()),d.css({top:Math.abs(parseInt(a[0])),left:Math.abs(parseInt(a[1]))}),this.loadMessages());c(function(){d.appendTo("body")});this.loadUi()},isToolbarLoaded:function(){return this.wrapper!==null},showToolbar:function(a){this.isToolbarLoaded()||this.loadToolbar();if(!this.visible){this.options.unify&&
this.hideOtherToolbars(true);this.visible=true;this.wrapper.css("display","");this.fire("resize");typeof this.getElement().attr("tabindex")==="undefined"&&this.getElement().attr("tabindex",-1);if(a)if(a.select)a.select();else{var b=j.getSelection();b.removeAllRanges();b.addRange(a)}var d=this;c(function(){d.fire("show");d.getElement().focus()})}},hideToolbar:function(){if(this.visible)this.visible=false,this.wrapper.hide(),this.fire("hide"),this.fire("resize")},hideOtherToolbars:function(a){this.unify(function(b){b.hideToolbar(a)},
...
{};for(var a in c.ui.editor.plugins)if(!(this.options.enablePlugins===false&&typeof this.options.plugins[a]==="undefined"||this.options.plugins[a]===false)&&c.inArray(a,this.options.disabledPlugins)===-1){var b=c.extend({},c.ui.editor.plugins[a]),d=a.replace(/([A-Z])/g,function(a){return"-"+a.toLowerCase()}),d=c.extend(true,{},this.options,{baseClass:this.options.baseClass+"-"+d},b.options,this.options.plugins[a]);b.editor=this;b.options=d;b.init(this,d);this.plugins[a]=b}},isDirty:function(){return this.dirty},
...
save:function(){var a=this.getCleanHtml();this.fire("save");this.setOriginalHtml(a);this.fire("saved");this.fire("change");return a},setOriginalHtml:function(a){this.originalHtml=a},bind:function(a,b,d){var e=this.events;c.each(a.split(","),function(a,h){h=c.trim(h);e[h]||(e[h]=[]);e[h].push({context:d,callback:b})})},unbind:function(a,b,d){for(var c=0,g=this.events[a].length;c<g;c++)this.events[a][c]&&this.events[a][c].callback===b&&this.events[a][c].context===d&&this.events[a].splice(c,1)},fire:function(a,
b,d){d||this.fire("before:"+a,b,true);if(this.events[a])for(var e=0,g=this.events[a].length;e<g;e++){var h=this.events[a][e];h.callback.call(h.context||this)}b!==false&&c.ui.editor.fire(a);d||this.fire("after:"+a,b,true)}});c.extend(c.ui.editor,{defaults:{plugins:{},ui:{},bind:{},domTools:{savedSelection:false,saveSelection:function(){this.savedSelection=i.saveSelection()},restoreSelection:function(){if(this.savedSelection)i.restoreSelection(this.savedSelection),this.savedSelection=false},eachRange:function(a,
b,d){for(var b=b||i.getSelection(),d=d||this,c,g=0;c=b.getAllRanges()[g++];)a.call(d,c)},constrainSelection:function(a,b){var a=c(a)[0],b=b||i.getSelection(),d;c(b.getAllRanges()).each(function(e,g){d=this.commonAncestorContainer.nodeType===3?c(g.commonAncestorContainer).parent()[0]:g.commonAncestorContainer;a!==d&&!c.contains(a,d)&&b.removeRange(g)})},getSelectedElements:function(){var a=new jQuery;this.eachRange(function(b){a.push(this.getSelectedElement(b)[0])});return a},getSelectedElement:function(a){return c(a.commonAncestorContainer.nodeType===
3?a.commonAncestorContainer.parentNode:a.commonAncestorContainer)},unwrapParentTag:function(a){this.getSelectedElements().each(function(){c(this).is(a)&&c(this).replaceWith(c(this).html())})},wrapTagWithAttribute:function(a,b,d){this.eachRange(function(c){c=this.getSelectedElement(c);c.is(a)?c.attr(b):this.toggleWrapper(a,{classes:d,attributes:b})})},selectInner:function(a,b){b=b||i.getSelection();b.removeAllRanges();c(a).focus().contents().each(function(){var a=i.createRange();a.selectNodeContents(this);
b.addRange(a)})},selectOuter:function(a,b){b=b||i.getSelection();b.removeAllRanges();c(a).each(function(){var a=i.createRange();a.selectNode(this);b.addRange(a)}).focus()},toggleWrapper:function(a,b){var b=b||{},d=i.createCssClassApplier(b.classes||"",{normalize:true,elementTagName:a,elementProperties:b.attributes||{}});this.eachRange(function(e){this.rangeEmptyTag(e)?this.replaceRange(c("<"+a+"/>").addClass(b.classes).attr(b.attributes||{}).append(this.domFragmentToHtml(e.cloneContents())),e):d.toggleRange(e)})},
rangeEmptyTag:function(a){a=this.domFragmentToHtml(a.cloneContents());typeof a==="string"&&(a=a.replace(/([ #;&,.+*~\':"!^$[\]()=>|\/@])/g,"\\$1"));return c(a).is(":empty")?true:false},execCommand:function(a,b,c){document.execCommand(a,b,c)},insertTag:function(a,b){this.eachRange(function(b){b.insertNode(c("<"+a+"/>")[0])},b)},insertTagAtEnd:function(a,b){this.eachRange(function(b){b.insertNodeAtEnd(c("<"+a+"/>")[0])},b)},insertElement:function(a,b,d){this.eachRange(function(d){c(a).each(function(){d.insertNode(b===
false?this:this.cloneNode(true))})},d)},insertElementAtEnd:function(a,b,d){this.eachRange(function(d){c(a).each(function(){d.insertNodeAtEnd(b===false?this:this.cloneNode(true))})},d)},toggleBlockStyle:function(a,b){this.eachRange(function(d){for(d=c(d.commonAncestorContainer);d.length&&d[0]!==b[0]&&(d[0].nodeType===3||d.css("display")==="inline");)d=d.parent();d[0]===b[0]&&b.css("display")!=="inline"&&(this.wrapInner(b,"div"),d=b.children().first());this.toggleStyle(d,a)})},wrapInner:function(a,
b){this.saveSelection();c(a).each(function(){var d=c("<"+b+"/>").html(c(this).html());a.html(d)});this.restoreSelection()},inverseWrapWithTagClass:function(a,b,d,e){this.saveSelection();var g="domTools"+Math.ceil(Math.random()*1E7);this.eachRange(function(a){var c=i.createCssClassApplier(e,{elementTagName:d});c.isAppliedToRange(a)?c.toggleSelection():i.createCssClassApplier(b,{elementTagName:g}).toggleSelection()});c(g).each(function(){c(this).replaceWith(c("<"+a+"/>").addClass(b).html(c(this).html()))});
this.restoreSelection()},toggleStyle:function(a,b){c.each(b,function(b,e){c(a).css(b)===e?c(a).css(b,""):c(a).css(b,e)})},getStyles:function(a){for(var b={},a=j.getComputedStyle(a[0],null),c=0;c<a.length;c++)b[a.item(c)]=a.getPropertyValue(a.item(c));return b},swapStyles:function(a,b,c){for(var e in c)a.css(e,b.css(e)),b.css(e,c[e])},replaceSelection:function(a,b){this.eachRange(function(b){this.replaceRange(a,b)},b)},replaceRange:function(a,b){var d=c("<div></div>").append(a)[0].childNodes;b.deleteContents();
d.length===p||d.length===1?b.insertNode(d[0].cloneNode(true)):c.each(d,function(a,c){b.insertNodeAtEnd(c.cloneNode(true))})},insertDomFragmentBefore:function(a,b,d){for(var e=0,g=a.childNodes.length;e<g;e++){var h=a.childNodes.item(e);(h=h.nodeType===3?h.nodeValue:c(h).html())&&c("<"+d+"/>").html(c.trim(h)).insertBefore(b)}},domFragmentToHtml:function(a,b){for(var d="",e=0,g=a.childNodes.length;e<g;e++){var h=a.childNodes.item(e);(h=h.nodeType===3?h.nodeValue:this.outerHtml(h))&&(d+=h)}b&&(d=c("<"+
b+">"+d+"</"+b+">"),d.find("p").wrapInner("<"+b+"/>"),d.find("p > *").unwrap(),d=c("<div></div>").html(d).html());return d},selectionExists:function(a){var b=false;this.eachRange(function(a){this.isEmpty(a)||(b=true)},a);return b},isEmpty:function(a){return a.startOffset===a.endOffset&&a.startContainer===a.endContainer},expandToParent:function(a){a.setStartBefore(a.startContainer);a.setEndAfter(a.endContainer)},changeTag:function(a,b){this.insertDomFragmentBefore(a.extractContents(),a.startContainer,
b);c(a.startContainer).remove()},tagSelection:function(a,b){this.tagSelectionWithin(a,null,b)},tagSelectionWithin:function(a,b,d){this.eachRange(function(d){this.isEmpty(d)?(this.expandToParent(d),b=c(b)[0],typeof b!=="undefined"&&d.startContainer===b&&d.endContainer===b?this.wrapInner(c(b),a):this.changeTag(d,a)):this.replaceRange(this.domFragmentToHtml(d.extractContents(),a),d)},d)},outerHtml:function(a){return c(a).clone().wrap("<div></div>").parent().html()}},namespace:null,unify:true,persistence:true,
...
c.ui.editor.registerPlugin("emptyElement",{options:{tag:"<p/>"},init:function(){this.bind("change",this.change)},change:function(){var a=this;this.textNodes(this.editor.getElement()).each(function(){c(this).wrap(c(a.options.tag))})},textNodes:function(a){return c(a).contents().filter(function(){return this.nodeType==3&&c.trim(this.nodeValue).length})}});c.ui.editor.registerUi({floatLeft:{init:function(a){return a.uiButton({title:f("Float Left"),click:function(){a.toggleBlockStyle({"float":"left"},
...
this.ui.enable()}}});c.ui.editor.registerPlugin("hotkeys",{options:{actions:[{ui:"textBold",key:"b",label:f("ctrl + b")},{ui:"textItalic",key:"i",label:f("ctrl + i")},{ui:"textUnderline",key:"u",label:f("ctrl + u")},{ui:"undo",key:"z",label:f("ctrl + z")},{ui:"redo",key:"y",label:f("ctrl + y")},{ui:"cancel",meta:false,key:27,label:f("esc")},{ui:"save",key:"s",label:f("ctrl + s")}]},indexedActions:{},keyUpEventSignature:null,keyDownEventSignature:null,init:function(a){this.keyUpEventSignature="keyup."+
this.options.baseClass;this.keyDownEventSignature="keydown."+this.options.baseClass;a.bind("enabled",this.enabled,this);a.bind("disabled",this.disabled,this)},disabled:function(){c(j).unbind(this.keyUpEventSignature);c(j).unbind(this.keyDownEventSignature);this.indexedActions={}},enabled:function(){for(var a,b=0;b<this.options.actions.length;b++)if(a=this.options.actions[b],this.indexedActions[this.isNumeric(a.key)?a.key:a.key.charCodeAt(0)]=a,typeof a.ui!=="undefined"){var d=this.editor.getUi(a.ui);
d.ui.button.attr("title",d.ui.title+" ("+a.label+")")}var e=this;c(j).bind(this.keyDownEventSignature,function(a){e.actionForKeyCombination.call(e,a)&&a.preventDefault()});c(j).bind(this.keyUpEventSignature,function(a){var b=e.actionForKeyCombination.call(e,a);b&&((c.isFunction(b.callback)?b.callback:function(){e.triggerUiAction(b.ui)}).call(e,a),a.preventDefault())})},actionForKeyCombination:function(a){var b=a.which;b>=65&&b<=90&&(b+=32);var c=this.indexedActions[b];if(typeof c==="undefined")return false;
if(!(c.meta===false||a.ctrlKey||a.metaKey))return false;a=false;return(a=this.isNumeric(c.key)?b===c.key:String.fromCharCode(b).toLowerCase()===c.key)?c:false},triggerUiAction:function(a,b){var c=this.editor.getUi(a);typeof c!=="undefined"&&c.ui.click.apply(c,b)},isNumeric:function(a){return!isNaN(a-0)}});c.ui.editor.registerUi({hr:{init:function(a){return a.uiButton({title:f("Insert Horizontal Rule"),click:function(){a.replaceSelection("<hr/>")}})}}});c.ui.editor.registerUi({i18n:{init:function(a){var b=
...
a.newValue.match(/width|height/)&&(this.options.resizeAjax&&b.addClass(this.options.resizeAjaxClass),this.editor.fire("change"))},scanImages:function(){var a=this.editor.getElement(),b=[];c(a.find("img")).each(function(){(a.height()<c(this).outerHeight()||a.width()<c(this).outerWidth())&&b.push(c(this))});b.length&&this.resizeImageElements(b,a.width(),a.height())},resizeImageElements:function(a,b,d){for(var e=c("<a>",{href:"",target:"_blank"}),g=0;g<a.length;g++){var f=a[g],k=f.outerWidth(),l=f.outerHeight(),
i=Math.min(b/k,d/l),k=Math.abs(i*(k-(f.outerWidth()-f.width()))),l=Math.abs(i*(l-(f.outerHeight()-f.height())));f.addClass(this.options.resizingClass);e=e.html(f.attr("title")||f.attr("src").substr(f.attr("src").lastIndexOf("/")+1)).attr("href",f.attr("src"));c(f).css({width:k,height:l}).attr("height",l).attr("width",k);this.options.resizeAjax&&f.addClass(this.options.resizeAjaxClass);var j=this;this.showOversizeWarning(this.editor.outerHtml(e),{hide:function(){f.removeClass(j.options.resizingClass)}})}},
...
[],e;for(e in b)d.push(e+"="+encodeURIComponent(b[e]));this.ui.button.find(".ui-button-icon-primary").css({"background-image":"url(http://www.jquery-raptor.com/logo/0.0.5?"+d.join("&")+")"})}})}}});c.ui.editor.registerPlugin("paste",{options:{allowedTags:"<h1>,<h2>,<h3>,<h4>,<h5>,<h6>,<div>,<ul>,<ol>,<li>,<blockquote>,<p>,<a>,<span>".split(",")},init:function(a,b){var d=false,e=false,g=this;a.getElement().bind("paste."+a.widgetName,c.proxy(function(){if(d)return false;d=true;a.saveSelection();c(".uiWidgetEditorPasteBin").length&&
c(".uiWidgetEditorPasteBin").remove();c('<div class="uiWidgetEditorPasteBin" contenteditable="true" style="width: 1px; height: 1px; overflow: hidden; position: fixed; top: -1px;" />').appendTo("body");c(".uiWidgetEditorPasteBin").focus();j.setTimeout(function(){var h=c(".uiWidgetEditorPasteBin").html(),h=g.filterAttributes(h),h=g.filterChars(h),h=g.stripTags(h),h=g.stripEmpty(h),h={html:h,plain:c("<div/>").html(h).text(),markup:g.stripAttributes(h)};e=c(a.getTemplate("paste.dialog",h));e.find(".ui-editor-paste-area").bind("keyup."+
a.widgetname,function(){g.updateAreas(this,e)});c(e).dialog({modal:true,width:650,height:500,resizable:true,title:"Paste",position:"center",show:b.dialogShowAnimation,hide:b.dialogHideAnimation,dialogClass:b.baseClass+" "+b.dialogClass,buttons:[{text:f("Insert"),click:function(){var b=null,b=c(this).find(".ui-editor-paste-area:visible"),b=b.hasClass("ui-editor-paste-plain")||b.hasClass("ui-editor-paste-source")?b.val():b.html(),b=g.filterAttributes(b),b=g.filterChars(b);a.restoreSelection();a.replaceSelection(b);
d=false;c(this).dialog("close")}},{text:f("Cancel"),click:function(){i.restoreSelection(selection);d=false;c(this).dialog("close")}}],open:function(){var a=c(this).find(".ui-editor-paste-panel-tabs");a.find("ul li").click(function(){a.find("ul li").removeClass("ui-state-active").removeClass("ui-tabs-selected");c(this).addClass("ui-state-active").addClass("ui-tabs-selected");a.children("div").hide().eq(c(this).index()).show()});var b=e.parent().find(".ui-dialog-buttonpane");b.find("button:eq(0)").button({icons:{primary:"ui-icon-circle-check"}});
...
handler:""}],function(b,c){a=a.replace(c.regexp,c.handler)});return a},filterChars:function(a){a=a.replace(/[\u2018|\u2019|\u201A]/g,"'");a=a.replace(/[\u201C|\u201D|\u201E]/g,'"');a=a.replace(/\u2026/g,"...");a=a.replace(/[\u2013|\u2014]/g,"-");a=a.replace(/\u02C6/g,"^");a=a.replace(/\u2039/g,"<");a=a.replace(/\u203A/g,">");return a=a.replace(/[\u02DC|\u00A0]/g," ")},stripAttributes:function(a){a=c("<div/>").html(a);c(a.find("*")).each(function(){var a=c.map(this.attributes,function(a){return a.name}),
d=c(this);c.each(a,function(a,b){try{d.removeAttr(b)}catch(c){}})});return a.html()},stripTags:function(a){allowed=(((this.options.allowedTags||"")+"").toLowerCase().match(/<[a-z][a-z0-9]*>/g)||[]).join("");return a.replace(/<\!--[\s\S]*?--\>|<\?(?:php)?[\s\S]*?\?>/gi,"").replace(/<\/?([a-z][a-z0-9]*)\b[^>]*>/gi,function(a,c){return allowed.indexOf("<"+c.toLowerCase()+">")>-1?a:""})},stripEmpty:function(a){a=c("<div/>").html(a);a.find("*").filter(function(){return c.trim(c(this).text())===""}).remove();
return a.html()},updateAreas:function(a,b){var d=b.find('.ui-editor-paste-synchronize-text input[type="checkbox"]'),e=c(a).is("textarea")?c(a).val():c(a).html();d.attr("checked")&&(c(a).hasClass("ui-editor-paste-plain")||b.find(".ui-editor-paste-plain").val(c("<div/>").html(e).text()),c(a).hasClass("ui-editor-paste-rich")||b.find(".ui-editor-paste-rich").html(e),c(a).hasClass("ui-editor-paste-source")||b.find(".ui-editor-paste-source").html(e),c(a).hasClass("ui-editor-paste-markup")||b.find(".ui-editor-paste-markup").html(this.stripAttributes(e)))}});
c.ui.editor.registerPlugin("placeholder",{init:function(a,b){b=c.extend({},{content:"[Your content here]",tag:"p",select:true},b);this.show=function(){if(!c.trim(a.getElement().html())){var d=c(document.createElement(b.tag)).html(b.content);a.getElement().html(d);b.select&&a.selectInner(d)}};a.bind("show",this.show)}});c.ui.editor.registerUi({raptorize:{init:function(a){var b=a.uiButton({title:f("Raptorize"),ready:function(){b.button.raptorize&&b.button.raptorize()}});return b}}});c.ui.editor.registerPlugin("saveJson",
...
e,a,d):c(e).appendTo(f);b.disableEditing();b.enableEditing()}}});c.ui.editor.registerUi({tagMenu:{init:function(a){a.bind("selectionChange",this.change,this);a.bind("show",this.change,this);var b=this;return a.uiSelectMenu({name:"tagMenu",title:f("Change HTML tag of selected element"),select:c(a.getTemplate("tagmenu.menu")),change:function(c){typeof c==="undefined"||c==="na"?b.change():a.tagSelectionWithin(c,a.getElement())}})},change:function(){var a=this.editor.getSelectedElements()[0];a&&(a=a.tagName.toLowerCase(),
this.ui.select.find("option[value="+a+"]").length?this.ui.val(a):this.ui.val("na"))}}});c.ui.editor.registerPlugin("toolbarTip",{init:function(){c.browser.msie||this.bind("show, tagTreeUpdated",function(){c(".ui-editor-wrapper [title]").each(function(){c(this).attr("data-title",c(this).attr("title"));c(this).removeAttr("title")})})}});(function(){var a=null,b=0;c.ui.editor.registerPlugin("unsavedEditWarning",{options:{},init:function(d){a||(a=c(d.getTemplate("unsavededitwarning.warning",this.options)).attr("id",
d.getUniqueId()).appendTo("body"));var e=this;d.bind("dirty",function(){b++;b>0&&a.addClass(e.options.baseClass+"-visible")});d.bind("cleaned",function(){b--;b===0&&a.removeClass(e.options.baseClass+"-visible")})},show:function(){},hide:function(){},reposition:function(){}})})();c.ui.editor.registerUi({viewSource:{dialog:null,init:function(a){a.bind("hide",this.hide,this);return a.uiButton({title:f("View / Edit Source"),click:function(){this.show()}})},tab:function(){return"    "},highlightSource:function(){return typeof j.Rainbow!==
"undefined"},untidyHtml:function(a){a=c("<div/>").html(a);c(a).find("span.support").each(function(){c(this).contents().unwrap()});a=a.html();a=a.replace(/&lt;/g,"<");a=a.replace(/&gt;/g,">");return a=a.replace(/&amp;/g,"&")},tidyHtml:function(a){if(typeof HTMLParser==="undefined")return a;var b=0,c="",e=this;HTMLParser(a,{start:function(a,f,i){for(var j=0;j<b;j++)c+=e.tab();c+="<"+a;for(j=0;j<f.length;j++)c+=" "+f[j].name+'="'+f[j].escaped+'"';c+=(i?"/":"")+">";b++},end:function(a){c+="</"+a+">";
b--},chars:function(a){c+=a},comment:function(a){c+="<\!--"+a+"--\>"}});return c},prepareSource:function(a){if(this.highlightSource()){var b=this.tidyHtml(c.trim(this.editor.getHtml())),a=c(a).find("."+this.options.baseClass+"-highlighted"),d=function(a,b,f){var a=c(a),k=a.find("code:first");k.unbind("keydown keyup");k.attr("contenteditable",true);Rainbow.color(b,"html",function(b){k.html(b);a.show();var g=function(){var a=i.getSelection();return a.rangeCount?a.getRangeAt(0):null},m="";k.bind("keydown",
function(a){m=k.html();switch(a.keyCode){case 13:var b=g(),c=false,d=document.createTextNode("\r\n");b&&(b.insertNode(d),b.setEndAfter(d),b.setStartAfter(d),c=i.getSelection(),c.setSingleRange(b),c=true);c&&a.preventDefault();return false;case 9:return b=g(),a=document.createTextNode(f.tab()),b&&(b.insertNode(a),c=i.getSelection(),b.setEndAfter(a),b.setStartAfter(a),c.setSingleRange(b)),false}});var n=false;k.bind("keydown",function(){n=true});k.bind("keyup",function(){n=false;j.setTimeout(function(){if(!n){if(m==
k.html())return true;try{var b=f.untidyHtml(k.html());c("<div/>").html(b).html()==b&&d(a,b,f)}catch(g){console.log("error")}}},5E3)})})};d(a,b,this)}else c(a).find("textarea").val(c.trim(this.editor.getHtml())),c(a).find("."+this.options.baseClass+"-plain-text").show()},hide:function(){this.dialog&&c(this.dialog).dialog("destroy").remove();this.dialog=null;c(this.ui.button).button("option","disabled",false)},show:function(){if(!this.dialog){c(this.ui.button).button("option","disabled",true);var a=
this;this.dialog=c(this.editor.getTemplate("viewsource.dialog",{baseClass:a.options.baseClass}));this.dialog.dialog({modal:false,width:600,height:400,resizable:true,title:f("View Source"),autoOpen:true,dialogClass:a.options.baseClass+" "+a.options.dialogClass,buttons:[{text:f("Apply Source"),click:function(){var b;a.highlightSource()?(b=c(this).find("."+a.options.baseClass+"-highlighted pre code:first").html(),b=a.untidyHtml(b)):b=c(this).find("textarea").val();a.editor.setHtml(b);c(this).find("textarea").val(a.editor.getHtml())}},
{text:f("Close"),click:function(){a.hide()}}],open:function(){var b=c(this).parent().find(".ui-dialog-buttonpane");b.find("button:eq(0)").button({icons:{primary:"ui-icon-circle-check"}});b.find("button:eq(1)").button({icons:{primary:"ui-icon-circle-close"}});a.prepareSource.call(a,this)},close:function(){a.hide()}})}}}})})(jQuery,window,rangy);
