...
function(a){var b=this.cloneRange();b.collapse(false);b.insertNode(a);b.detach();this.setEndAfter(a)}});c("html").click(function(){c(".ui-editor-selectmenu-visible").removeClass("ui-editor-selectmenu-visible")});c.widget("ui.editor",{_init:function(){c.inArray(this,c.ui.editor.instances)===-1&&c.ui.editor.instances.push(this);var a=this;c.ui.editor.eachInstance(function(b){a!=b&&a.element.closest(b.element).length&&handleError("Nesting editors is unsupported",a.element,b.element)});this.options=c.extend({},
c.ui.editor.defaults,this.options);this.options.uiOrder=this.options.uiOrder||[["logo"],["save","cancel"],["dock","showGuides","clean"],["viewSource"],["undo","redo"],["alignLeft","alignCenter","alignJustify","alignRight"],["textBold","textItalic","textUnderline","textStrike"],["textSuper","textSub"],["listUnordered","listOrdered"],["hr","quoteBlock"],["fontSizeInc","fontSizeDec"],["link","unlink"],["embed"],["floatLeft","floatNone","floatRight"],["tagMenu"],["i18n"],["raptorize"],["length"],["debugReinit",
"debugDestroy"]];this.element.attr("id")||this.element.attr("id",this.getUniqueId());this.ready=false;this.events={};this.ui={};this.plugins={};this.templates=c.extend({},c.ui.editor.templates);this.path=this.toolbarWrapper=this.toolbar=this.wrapper=null;this.visible=this.enabled=false;this.uiObjects={};for(var b in this.options.bind)this.bind(b,this.options.bind[b]);this.history=[];this.present=0;this.historyEnabled=true;if(r(this)){this.cloneDomTools();this.setOriginalHtml(this.element.is(":input")?
this.element.val():this.element.html());if(this.options.replace)this.replaceOriginal(),this.options.replace=false;this.attach();this.loadPlugins();this.dirty=false;this.previousSelection=this.previousContent=null;this.ready=true;this.fire("ready");this.options.autoEnable&&c(function(){a.enableEditing();a.showToolbar()})}},attach:function(){this.bind("change",this.historyPush);this.bind("selectionChange",this.updateTagTree);this.bind("show",this.updateTagTree);var a=c.proxy(this.checkChange,this);
this.getElement().find("img").bind("click."+this.widgetName,c.proxy(function(a){this.selectOuter(a.target)},this));this.getElement().bind("mouseup."+this.widgetName,a);this.getElement().bind("keyup."+this.widgetName,a);c(k).bind("beforeunload",c.proxy(c.ui.editor.unloadWarning,c.ui.editor))},reinit:function(){if(this.ready){var a=this.enabled,b=this.visible;this.destruct();this._init();a&&this.enableEditing();b&&this.showToolbar()}else{var d;d=function(){this.unbind("ready",d);this.reinit()};this.bind("ready",
d)}},getElement:function(){return this.target?this.target:this.element},getOriginalElement:function(){return this.element},replaceOriginal:function(){if(!this.target){for(var a=c("<div/>").html(this.element.is(":input")?this.element.val():this.element.html()).insertBefore(this.element).attr("id",this.getUniqueId()).addClass(this.element.attr("class")),b=this.options.domTools.getStyles(this.element),d=0;d<this.options.replaceStyle.length;d++)a.css(this.options.replaceStyle[d],b[this.options.replaceStyle[d]]);
this.element.hide();this.bind("change",function(){this.element.is("input")?this.element.val(this.getHtml()):this.element.html(this.getHtml())});this.target=a}},cloneDomTools:function(){for(var a in this.options.domTools)this[a]||(this[a]=function(a){return function(){this.options.domTools.constrainSelection(this.getElement());var d=this.getHtml(),c=this.options.domTools[a].apply(this.options.domTools,arguments);d!==this.getHtml()&&this.change();return c}}(a))},checkChange:function(){var a=i.serializeSelection();
...
false,false),document.execCommand("styleWithCSS",true,true)}catch(a){}this.fire("enabled");this.fire("resize")}},disableEditing:function(){if(this.enabled)this.enabled=false,this.getElement().attr("contenteditable",false).removeClass(this.options.baseClass+"-editing"),this.fire("disabled")},isEditing:function(){return this.enabled},updateTagTree:function(){if(this.isEditing()){var a=this,b="",d=[],e=0;this.eachRange(function(g){var g=g.commonAncestorContainer,h;h=g.nodeType===3?c(g).parent():c(g);
g=[];for(d.push(g);h[0]&&!a.isRoot(h)&&h[0].tagName.toLowerCase()!=="body";)g.push(h),h=h.parent();g.reverse();b&&(b+=" | ");b+=this.getTemplate("root");for(h=0;h<g.length;h++)b+=this.getTemplate("tag",{element:g[h][0].tagName.toLowerCase(),data:"["+e+","+h+"]"});e++},null,this);b||(b=this.getTemplate("root"));this.path.html(b).find("a").click(function(){var b=c(this).data("ui-editor-selection");b?(a.selectOuter(d[b[0]][b[1]]),a.updateTagTree()):a.selectOuter(a.getElement())});this.fire("tagTreeUpdated")}},
isRoot:function(a){return this.getElement()[0]===c(a)[0]},unify:function(a,b){b!==false&&a(this);if(this.options.unify)for(var d=c.ui.editor.getInstances(),e=0;e<d.length;e++)d[e]!==this&&d[e].options.unify&&a(d[e])},getUniqueId:function(){return c.ui.editor.getUniqueId()},loadMessages:function(){this.messages=c(this.getTemplate("messages")).appendTo(this.wrapper)},showMessage:function(a,b,d){var d=c.extend({},this.options.message,d),e={timer:null,editor:this,show:function(){this.element.slideDown();
this.timer=k.setTimeout(function(a){this.timer=null;a.hide()},d.delay,this)},hide:function(){if(this.timer)k.clearTimeout(this.timer),this.timer=null;this.element.stop().slideUp(c.proxy(function(){c.isFunction(d.hide)&&d.hide.call(this);this.element.remove()},this))}};e.element=c(this.getTemplate("message",{type:a,message:b})).hide().appendTo(this.messages).find(".ui-editor-message-close").click(function(){e.hide()}).end();e.show();return e},showLoading:function(a,b){return this.showMessage("clock",
...
c("<div/>").addClass(this.options.baseClass+"-path").addClass("ui-widget-header").html(this.getTemplate("root")),d=this.wrapper=c("<div/>").addClass(this.options.baseClass+"-wrapper").css("display","none").append(b).append(a);c.fn.draggable&&this.options.draggable&&(d.draggable({cancel:"a, button",cursor:"move",handle:".ui-editor-path",stop:c.proxy(function(){var a=this.persist("position",[d.css("top"),d.css("left")]);d.css({top:Math.abs(a[0]),left:Math.abs(a[1])})},this)}),d.css("position",""),(a=
this.persist("position")||this.options.dialogPosition)||(a=[10,10]),parseInt(a[0],10)+d.outerHeight()>c(k).height()&&(a[0]=c(k).height()-d.outerHeight()),parseInt(a[1],10)+d.outerWidth()>c(k).width()&&(a[1]=c(k).width()-d.outerWidth()),d.css({top:Math.abs(parseInt(a[0])),left:Math.abs(parseInt(a[1]))}),this.loadMessages());c(function(){d.appendTo("body")});this.loadUi()},isToolbarLoaded:function(){return this.wrapper!==null},showToolbar:function(a){this.isToolbarLoaded()||this.loadToolbar();if(!this.visible){this.options.unify&&
this.hideOtherToolbars(true);this.visible=true;this.wrapper.css("display","");this.fire("resize");typeof this.getElement().attr("tabindex")==="undefined"&&this.getElement().attr("tabindex",-1);if(a)if(a.select)a.select();else{var b=k.getSelection();b.removeAllRanges();b.addRange(a)}var d=this;c(function(){d.fire("show");d.getElement().focus()})}},hideToolbar:function(){if(this.visible)this.visible=false,this.wrapper.hide(),this.fire("hide"),this.fire("resize")},hideOtherToolbars:function(a){this.unify(function(b){b.hideToolbar(a)},
false)},isVisible:function(){return this.visible},getTemplate:function(a,b){var d;d=this.templates[a]?this.templates[a]:c.ui.editor.getTemplate(a,this.options.urlPrefix);d=d.replace(/_\('(.*?)'\)/g,function(a,b){b=b.replace(/\\(.?)/g,function(a,b){switch(b){case "\\":return"\\";case "0":return"\x00";case "":return"";default:return b}});return f(b)});b=c.extend({},this.options,b||{});b=this.getTemplateVars(b);return d=d.replace(/\{\{(.*?)\}\}/g,function(a,d){return b[d]})},getTemplateVars:function(a,
b,d){b=b?b+".":"";d||(d=1);var c={},g;for(g in a)if(typeof a[g]==="object"&&d<5){var h=this.getTemplateVars(a[g],b+g,++d),f;for(f in h)c[f]=h[f]}else c[b+g]=a[g];return c},historyPush:function(){if(this.historyEnabled&&this.getHtml()!==this.historyPeak()){if(this.present!==this.history.length-1)this.history=this.history.splice(0,this.present+1);this.history.push(this.getHtml());this.present=this.history.length-1}},historyPeak:function(){return!this.history.length?null:this.history[this.present]},
historyBack:function(){if(this.present>0)this.present--,this.setHtml(this.history[this.present]),this.historyEnabled=false,this.change(),this.historyEnabled=true},historyForward:function(){if(this.present<this.history.length-1)this.present++,this.setHtml(this.history[this.present]),this.historyEnabled=false,this.change(),this.historyEnabled=true},loadUi:function(){for(var a=0,b=this.options.uiOrder.length;a<b;a++){for(var d=this.options.uiOrder[a],e=c("<div/>"),g=0,h=d.length;g<h;g++)if(!(this.options.enableUi===
false&&typeof this.options.ui[d[g]]==="undefined"||this.options.ui[d[g]]===false)&&c.inArray(d[g],this.options.disabledUi)===-1){var f=d[g].replace(/([A-Z])/g,function(a){return"-"+a.toLowerCase()});if(c.ui.editor.ui[d[g]]){var l=c.extend({},c.ui.editor.ui[d[g]]),f=c.extend(true,{},this.options,{baseClass:this.options.baseClass+"-ui-"+f},l.options,this.options.ui[d[g]]);l.editor=this;l.options=f;l.ui=l.init(this,f);l.ui.init(d[g],this,f,l).appendTo(e);this.uiObjects[d[g]]=l}}e.addClass("ui-buttonset").addClass(this.options.baseClass+
"-buttonset");e.children().length>0&&e.appendTo(this.toolbar)}c("<div/>").css("clear","both").appendTo(this.toolbar)},uiButton:function(a){return c.extend({button:null,options:{},init:function(a,d,e,g){var h=a.replace(/([A-Z])/g,function(a){return"-"+a.toLowerCase()}),e=c.extend({},d.options,{baseClass:d.options.baseClass+"-"+h+"-button"},this.options,d.options.ui[a]);if(!this.title)this.title=f("Unnamed Button");this.button=c("<div/>").html(this.label||this.title).addClass(e.baseClass).attr("name",
a).attr("title",this.title).val(a);e.classes&&this.button.addClass(e.classes);this.button.bind("mousedown."+g.editor.widgetName,function(a){a.preventDefault()});var j=this;this.button.bind("mouseup."+g.editor.widgetName,function(a){a.preventDefault();j.click.apply(g,arguments)});d.bind("destroy",c.proxy(function(){this.button.button("destroy").remove()},this));this.button.button({icons:{primary:this.icon||"ui-icon-"+h},disabled:e.disabled?true:false,text:this.text||false,label:this.label||null});
this.ready.call(g);return this.button},disable:function(){this.button.button("option","disabled",true)},enable:function(){this.button.button("option","disabled",false)},ready:function(){},click:function(){}},a)},uiSelectMenu:function(a){return c.extend({select:null,selectMenu:null,button:null,menu:null,options:{},init:function(a,d){var e=this,g=a.replace(/([A-Z])/g,function(a){return"-"+a.toLowerCase()});c.extend({},d.options,{baseClass:d.options.baseClass+g+"-select-menu"},e.options,d.options.ui[a]);
if(!e.title)e.title=f("Unnamed Select Menu");e.wrapper=c('<div class="ui-editor-selectmenu-wrapper"/>').append(e.select.hide());e.selectMenu=c('<div class="ui-selectmenu ui-editor-selectmenu"/>').appendTo(e.wrapper);e.menu=c('<div class="ui-selectmenu-menu ui-editor-selectmenu-menu ui-widget-content ui-corner-bottom ui-corner-tr"/>').appendTo(e.wrapper);e.select.find("option").each(function(){c("<div/>").addClass("ui-selectmenu-menu-item ui-editor-selectmenu-menu-item").addClass("ui-corner-all").html(c(this).html()).appendTo(e.menu).bind("mouseenter."+
...
e.button=c("<div/>").addClass("ui-selectmenu-button ui-editor-selectmenu-button ui-button ui-state-default").attr("title",e.title).append(g).append(h).prependTo(e.selectMenu);e.button.bind("click."+d.widgetName,function(){e.menu.css("min-width",e.button.outerWidth()+10);e.wrapper.toggleClass("ui-editor-selectmenu-visible");return false});g=e.select.find("option[value="+e.select.val()+"]").html();e.button.find(".ui-selectmenu-text").html(g);return e.wrapper},update:function(){var a=this.select.find("option[value="+
this.select.val()+"]").html();this.button.find(".ui-selectmenu-text").html(a)},val:function(){var a=this.select.val.apply(this.select,arguments);this.update();return a},change:function(){}},a)},getPlugin:function(a){return this.plugins[a]},loadPlugins:function(){if(!this.options.plugins)this.options.plugins={};for(var a in c.ui.editor.plugins)if(!(this.options.enablePlugins===false&&typeof this.options.plugins[a]==="undefined"||this.options.plugins[a]===false)&&c.inArray(a,this.options.disabledPlugins)===
-1){var b=c.extend({},c.ui.editor.plugins[a]),d=a.replace(/([A-Z])/g,function(a){return"-"+a.toLowerCase()}),d=c.extend(true,{},this.options,{baseClass:this.options.baseClass+"-"+d},b.options,this.options.plugins[a]);b.editor=this;b.options=d;b.init(this,d);this.plugins[a]=b}},isDirty:function(){return this.dirty},getHtml:function(){var a=this.getElement().html(),a=c("<div/>").html(a);a.find(".rangySelectionBoundary").remove();return a=a.html()},getCleanHtml:function(){this.fire("clean");var a=this.getElement().html();
this.fire("restore");a=c("<div/>").html(a);a.find(".rangySelectionBoundary").remove();return a=a.html()},setHtml:function(a){this.getElement().html(a);this.fire("html");this.change()},resetHtml:function(){this.setHtml(this.getOriginalHtml())},getOriginalHtml:function(){return this.originalHtml},save:function(){var a=this.getCleanHtml();this.fire("save");this.setOriginalHtml(a);this.fire("saved");this.fire("change");return a},setOriginalHtml:function(a){this.originalHtml=a},bind:function(a,b,d){var e=
this.events;c.each(a.split(","),function(a,f){f=c.trim(f);e[f]||(e[f]=[]);e[f].push({context:d,callback:b})})},unbind:function(a,b,d){for(var c=0,g=this.events[a].length;c<g;c++)this.events[a][c]&&this.events[a][c].callback===b&&this.events[a][c].context===d&&this.events[a].splice(c,1)},fire:function(a,b,d){d||this.fire("before:"+a,b,true);if(this.events[a])for(var e=0,g=this.events[a].length;e<g;e++){var f=this.events[a][e];f.callback.call(f.context||this)}b!==false&&c.ui.editor.fire(a);d||this.fire("after:"+
a,b,true)}});c.extend(c.ui.editor,{defaults:{plugins:{},ui:{},bind:{},domTools:{savedSelection:false,saveSelection:function(){this.savedSelection=i.saveSelection()},restoreSelection:function(){if(this.savedSelection)i.restoreSelection(this.savedSelection),this.savedSelection=false},eachRange:function(a,b,d){for(var b=b||i.getSelection(),d=d||this,c,g=0;c=b.getAllRanges()[g++];)a.call(d,c)},constrainSelection:function(a,b){var a=c(a)[0],b=b||i.getSelection(),d;c(b.getAllRanges()).each(function(e,g){d=
this.commonAncestorContainer.nodeType===3?c(g.commonAncestorContainer).parent()[0]:g.commonAncestorContainer;a!==d&&!c.contains(a,d)&&b.removeRange(g)})},getSelectedElements:function(){var a=new jQuery;this.eachRange(function(b){a.push(this.getSelectedElement(b)[0])});return a},getSelectedElement:function(a){return c(a.commonAncestorContainer.nodeType===3?a.commonAncestorContainer.parentNode:a.commonAncestorContainer)},unwrapParentTag:function(a){this.getSelectedElements().each(function(){c(this).is(a)&&
...
this.dialog=c(this.editor.getTemplate("embed.dialog"));this.dialog.dialog({modal:false,width:600,height:400,resizable:true,title:f("Paste Embed Code"),autoOpen:true,dialogClass:a.options.baseClass+" "+a.options.dialogClass,buttons:[{text:f("Embed Object"),click:function(){a.editor.restoreSelection();a.editor.replaceSelection(c(this).find("textarea").val());c(this).dialog("close")}},{text:f("Close"),click:function(){a.hide()}}],open:function(){var a=c(this).parent().find(".ui-dialog-buttonpane");a.find("button:eq(0)").button({icons:{primary:"ui-icon-circle-check"}});
...
c.ui.editor.registerUi({floatLeft:{init:function(a){return a.uiButton({title:f("Float Left"),click:function(){a.applyStyle({"float":"left"})}})}},floatRight:{init:function(a){return a.uiButton({title:f("Float Right"),click:function(){a.applyStyle({"float":"right"})}})}},floatNone:{init:function(a){return a.uiButton({title:f("Float None"),click:function(){a.applyStyle({"float":"none"})}})}}});c.ui.editor.registerUi({fontSizeInc:{init:function(a,b){return a.uiButton({title:f("Increase Font Size"),click:function(){a.inverseWrapWithTagClass("big",
...
this.ui.enable()}}});c.ui.editor.registerUi({hr:{init:function(a){return a.uiButton({title:f("Insert Horizontal Rule"),click:function(){a.replaceSelection("<hr/>")}})}}});c.ui.editor.registerUi({i18n:{init:function(a){var b=this,d=this.persist("locale");d&&n(d);var d=c('<select autocomplete="off" name="i18n"/>'),e;for(e in q){var g=c('<option value="'+e+'" class="'+e+'"/>');g.html(t[e]);m===e&&g.attr("selected","selected");d.append(g)}return a.uiSelectMenu({title:f("Change Language"),select:d,change:function(a){n(b.persist("locale",
a))}})}}});o("en","English",{});o("zh_CN","\u7b80\u4f53\u4e2d\u6587",{"Added link: {{link}}":"Added link: {{link}}","All changes will be lost!":"All changes will be lost!","Apply Source":"\u5e94\u7528\u6e90\u4ee3\u7801","Are you sure you want to stop editing?":"Are you sure you want to stop editing?",Blockquote:"\u5927\u6bb5\u5f15\u7528",Bold:"\u7c97\u4f53",Cancel:"\u53d6\u6d88","Center Align":"\u4e2d\u5fc3\u5bf9\u9f50\u6587\u672c","Change HTML tag of selected element":"Change HTML tag of selected element",
...
a.newValue.match(/width|height/)&&(this.options.resizeAjax&&b.addClass(this.options.resizeAjaxClass),this.editor.fire("change"))},scanImages:function(){var a=this.editor.getElement(),b=[];c(a.find("img")).each(function(){(a.height()<c(this).outerHeight()||a.width()<c(this).outerWidth())&&b.push(c(this))});b.length&&this.resizeImageElements(b,a.width(),a.height())},resizeImageElements:function(a,b,d){for(var e=c("<a>",{href:"",target:"_blank"}),g=0;g<a.length;g++){var f=a[g],j=f.outerWidth(),l=f.outerHeight(),
i=Math.min(b/j,d/l),j=Math.abs(i*(j-(f.outerWidth()-f.width()))),l=Math.abs(i*(l-(f.outerHeight()-f.height())));f.addClass(this.options.resizingClass);e=e.html(f.attr("title")||f.attr("src").substr(f.attr("src").lastIndexOf("/")+1)).attr("href",f.attr("src"));c(f).css({width:j,height:l}).attr("height",l).attr("width",j);this.options.resizeAjax&&f.addClass(this.options.resizeAjaxClass);var k=this;this.showOversizeWarning(this.editor.outerHtml(e),{hide:function(){f.removeClass(k.options.resizingClass)}})}},
...
click:function(){this.show()}})},tab:function(){return"    "},highlightSource:function(){return typeof k.Rainbow!=="undefined"},untidyHtml:function(a){a=c("<div/>").html(a);c(a).find("span.support").each(function(){c(this).contents().unwrap()});a=a.html();a=a.replace(/&lt;/g,"<");a=a.replace(/&gt;/g,">");return a=a.replace(/&amp;/g,"&")},tidyHtml:function(a){if(typeof HTMLParser==="undefined")return a;var b=0,c="",e=this;HTMLParser(a,{start:function(a,f,i){for(var l=0;l<b;l++)c+=e.tab();c+="<"+a;
for(l=0;l<f.length;l++)c+=" "+f[l].name+'="'+f[l].escaped+'"';c+=(i?"/":"")+">";b++},end:function(a){c+="</"+a+">";b--},chars:function(a){c+=a},comment:function(a){c+="<\!--"+a+"--\>"}});return c},prepareSource:function(a){if(this.highlightSource()){var b=this.tidyHtml(c.trim(this.editor.getHtml())),a=c(a).find("."+this.options.baseClass+"-highlighted"),d=function(a,b,f){var a=c(a),j=a.find("code:first");j.unbind("keydown keyup");j.attr("contenteditable",true);Rainbow.color(b,"html",function(b){j.html(b);
a.show();var g=function(){var a=i.getSelection();return a.rangeCount?a.getRangeAt(0):null},m="";j.bind("keydown",function(a){m=j.html();switch(a.keyCode){case 13:var b=g(),c=false,d=document.createTextNode("\r\n");b&&(b.insertNode(d),b.setEndAfter(d),b.setStartAfter(d),c=i.getSelection(),c.setSingleRange(b),c=true);c&&a.preventDefault();return false;case 9:return b=g(),a=document.createTextNode(f.tab()),b&&(b.insertNode(a),c=i.getSelection(),b.setEndAfter(a),b.setStartAfter(a),c.setSingleRange(b)),
false}});var n=false;j.bind("keydown",function(){n=true});j.bind("keyup",function(){n=false;k.setTimeout(function(){if(!n){if(m==j.html())return true;try{var b=f.untidyHtml(j.html());c("<div/>").html(b).html()==b&&d(a,b,f)}catch(g){console.log("error")}}},5E3)})})};d(a,b,this)}else c(a).find("textarea").val(c.trim(this.editor.getHtml())),c(a).find("."+this.options.baseClass+"-plain-text").show()},hide:function(){this.dialog&&c(this.dialog).dialog("destroy").remove();this.dialog=null;c(this.ui.button).button("option",
"disabled",false)},show:function(){if(!this.dialog){c(this.ui.button).button("option","disabled",true);var a=this;this.dialog=c(this.editor.getTemplate("viewsource.dialog",{baseClass:a.options.baseClass}));this.dialog.dialog({modal:false,width:600,height:400,resizable:true,title:f("View Source"),autoOpen:true,dialogClass:a.options.baseClass+" "+a.options.dialogClass,buttons:[{text:f("Apply Source"),click:function(){var b;a.highlightSource()?(b=c(this).find("."+a.options.baseClass+"-highlighted pre code:first").html(),
b=a.untidyHtml(b)):b=c(this).find("textarea").val();a.editor.setHtml(b)}},{text:f("Close"),click:function(){a.hide()}}],open:function(){var b=c(this).parent().find(".ui-dialog-buttonpane");b.find("button:eq(0)").button({icons:{primary:"ui-icon-circle-check"}});b.find("button:eq(1)").button({icons:{primary:"ui-icon-circle-close"}});a.prepareSource.call(a,this)},close:function(){a.hide()}})}}}})})(jQuery,window,rangy);
