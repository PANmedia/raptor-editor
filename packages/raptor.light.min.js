...
this.history=[];this.present=0;this.historyEnabled=true;if(w(this)){this.cloneDomTools();this.setOriginalHtml(this.element.is(":input")?this.element.val():this.element.html());if(this.options.replace)this.replaceOriginal(),this.options.replace=false;this.attach();this.loadPlugins();this.dirty=false;this.previousSelection=this.previousContent=null;this.ready=true;this.fire("ready");this.options.autoEnable&&d(function(){a.enableEditing();a.showToolbar()})}},attach:function(){this.bind("change",this.historyPush);
this.bind("selectionChange",this.updateTagTree);this.bind("show",this.updateTagTree);var a=d.proxy(this.checkChange,this);this.getElement().find("img").bind("click."+this.widgetName,d.proxy(function(a){this.selectOuter(a.target)},this));this.getElement().bind("mouseup."+this.widgetName,a);this.getElement().bind("keyup."+this.widgetName,a);d(q).bind("beforeunload",d.proxy(d.ui.editor.unloadWarning,d.ui.editor))},reinit:function(){if(this.ready){var a=this.enabled,b=this.visible;this.destruct();this._init();
...
"-toolbar-wrapper").addClass("ui-widget-content").append(a),b=this.path=d("<div/>").addClass(this.options.baseClass+"-path").addClass("ui-widget-header").html(this.getTemplate("root")),c=this.wrapper=d("<div/>").addClass(this.options.baseClass+"-wrapper").css("display","none").append(b).append(a);d.fn.draggable&&this.options.draggable&&(c.draggable({cancel:"a, button",cursor:"move",handle:".ui-editor-path",stop:d.proxy(function(){var a=this.persist("position",[c.css("top"),c.css("left")]);c.css({top:Math.abs(a[0]),
...
c.options,{baseClass:c.options.baseClass+g+"-select-menu"},e.options,c.options.ui[a]);if(!e.title)e.title=f("Unnamed Select Menu");e.wrapper=d('<div class="ui-editor-selectmenu-wrapper"/>').append(e.select.hide());e.selectMenu=d('<div class="ui-selectmenu ui-editor-selectmenu"/>').appendTo(e.wrapper);e.menu=d('<div class="ui-selectmenu-menu ui-editor-selectmenu-menu ui-widget-content ui-corner-bottom ui-corner-tr"/>').appendTo(e.wrapper);e.select.find("option").each(function(){d("<div/>").addClass("ui-selectmenu-menu-item ui-editor-selectmenu-menu-item").addClass("ui-corner-all").html(d(this).html()).appendTo(e.menu).bind("mouseenter."+
...
this.ui.enable()}}});d.ui.editor.registerUi({hr:{init:function(a){return a.uiButton({title:f("Insert Horizontal Rule"),click:function(){a.replaceSelection("<hr/>")}})}}});d.ui.editor.registerUi({i18n:{init:function(a){var b=this,c=this.persist("locale");c&&s(c);var c=d('<select autocomplete="off" name="i18n"/>'),e;for(e in o){var g=d('<option value="'+e+'" class="'+e+'"/>');g.html(h[e]);m===e&&g.attr("selected","selected");c.append(g)}return a.uiSelectMenu({title:f("Change Language"),select:c,change:function(a){s(b.persist("locale",
