...
(function(d,q,k,t){function r(a,b,c){p[a]=c;h[a]=b;n||(n=a)}function s(a){n!==a&&(n=a,d.ui.editor.eachInstance(function(){this.reinit()}))}function f(a,b){n&&p[n]&&p[n][a]&&(a=p[n][a]);if(b)for(var c in b)a=a.replace("{{"+c+"}}",b[c]);return a}function w(a){if(m===t){m=true;if(l=/(iPhone|iPod|iPad).*AppleWebKit/i.test(navigator.userAgent))d("html").addClass(a.options.baseClass+"-ios"),l&&d(document).bind("scroll",function(){setInterval(function(){d("body").css("height","+=1").css("height","-=1")},
0)});d.browser.mozilla&&d("html").addClass(a.options.baseClass+"-ff");if(d.browser.msie&&d.browser.version<9){m=false;var b=d("<div></div>").addClass(a.options.baseClass+"-unsupported").html(a.getTemplate("unsupported")).appendTo("body"),c=1;b.siblings().each(function(){var a=d(this).css("z-index");!isNaN(a)&&a>c&&(c=a+1)});b.css("z-index",c);b.find("."+a.options.baseClass+"-unsupported-close").click(function(){b.remove()})}}return m}var n=null,p={},h={};d(function(){k.initialized||k.init();if(!d.isFunction(k.rangePrototype.insertNodeAtEnd))k.rangePrototype.insertNodeAtEnd=
